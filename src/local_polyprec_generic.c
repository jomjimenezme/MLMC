...
int local_fgmres_double( local_gmres_double_struct *p, level_struct *l, struct Thread *threading ) {
  ...
  gamma0 = process_norm_double( p->r, start, end, l, threading );

  START_MASTER(threading)
  p->gamma[0] = gamma0;
  END_MASTER(threading)
  norm_r0 = creal(gamma0);
  vector_double_real_scale( p->V[0], p->r, 1.0/gamma0, start, end, l ); // v_0 = r / gamma_0
  ...
  for( il=0; il<p->restart_length && finish==0; il++) {
    ...
    if ( !local_arnoldi_step_double( p->V, p->Z, p->w, p->H, p->y, j, NULL, p, l, threading ) ) {
      printf0("| -------------- iteration %d, restart due to H(%d,%d) < 0 |\n", iter, j+1, j );
      break;
    }

    if ( cabs( p->H[j][j+1] ) > p->tol/10 ) {
      local_qr_update_double( p->H, p->s, p->c, p->gamma, j, l, threading );
...
int local_arnoldi_step_double( vector_double *V, vector_double *Z, vector_double w,
  ...
  process_multi_inner_product_double( j+1, tmp, V, w, start, end, l, threading );
  ...
  double tmp2 = process_norm_double( w, start, end, l, threading );
  ...
  // V_j+1 = w / H_j+1,j
  if ( cabs_double( tmp2 ) > 1e-15 )
    vector_double_real_scale( V[j+1], w, 1/tmp2, start, end, l );
...
void local_set_ghost_double( vector_double phi, const int mu, const int dir,
  ...
  if( l->global_splitting[mu] > 1 ) {
    ...
    // dir = senddir
    if ( dir == 1 ) {
      memset( buffer, 0.0, length[1]*sizeof(complex_double) );
    } else if ( dir == -1 ) {
      phi_pt = phi + comm_start;
      memset( phi_pt, 0.0, length[0]*sizeof(complex_double) );
...
void coarse_local_n_hopping_term_double( vector_double out, vector_double in, operator_double_struct *op,
  ...
  memset( out+l->inner_vector_size, 0.0, (l->vector_size-l->inner_vector_size)*sizeof(complex_double) );
  ...
  if ( op->c.comm ) {
    for ( mu=0; mu<4; mu++ ) {
      local_set_ghost_double( in, mu, -1, &(op->c), minus_dir_param, l );
    }
  }
  if ( op->c.comm ) {
    for ( mu=0; mu<4; mu++ ) {
      local_set_ghost_double( out, mu, +1, &(op->c), plus_dir_param, l );    
    }
  }

  //memset( in+l->inner_vector_size, 0.0, (l->vector_size-l->inner_vector_size)*sizeof(complex_double) );
  //memset( out+l->inner_vector_size, 0.0, (l->vector_size-l->inner_vector_size)*sizeof(complex_double) );
...
void coarse_local_apply_schur_complement_double( vector_double out, vector_double in,
  ...
  //coarse_local_diag_ee_double( out, in, op, l, threading );
  coarse_diag_ee_double( out, in, op, l, threading );

  vector_double_define( tmp[0], 0, start, end, l );

  //coarse_local_hopping_term_double( tmp[0], in, op, _ODD_SITES, l, threading );
  //coarse_local_diag_oo_inv_double( tmp[1], tmp[0], op, l, threading );
  //coarse_local_n_hopping_term_double( out, tmp[1], op, _EVEN_SITES, l, threading );

  //coarse_local_diag_oo_inv_double( tmp[0], in, op, l, threading );
  coarse_diag_oo_inv_double( tmp[0], in, op, l, threading );

  vector_double_minus( out, out, tmp[0], start, end, l );
...
void local_update_lejas_double( local_gmres_double_struct *p, level_struct *l, struct Thread *threading )
{
  ...
  local_fgmres_double(p, l, threading);
  ...
  // TODO : re-enable this
  p->polyprec_double.update_lejas = 0;
  l->p_double.block_jacobi_double.BJ_usable = 1;
  END_MASTER(threading)

  START_MASTER(threading)
  local_harmonic_ritz_double(p);
  local_leja_ordering_double(p);
  END_MASTER(threading)
}
...
void local_re_construct_lejas_double( level_struct *l, struct Thread *threading ) {
  ...
  local_update_lejas_double(&(l->p_double.block_jacobi_double.local_p), l, threading);

  local_gmres_double_struct *p = &(l->p_double.block_jacobi_double.local_p);

  START_MASTER(threading)
  int d_poly = p->polyprec_double.d_poly;
  printf0("lejas (proc=%d) =  ", g.my_rank);
  for ( int i=0; i<d_poly; i++ ) {
    printf0( "%f+%fj  ", creal(p->polyprec_double.lejas[i]), cimag(p->polyprec_double.lejas[i]) );
  }
  printf0("\n");
  END_MASTER(threading)
                                     ...
                                     int res, level_struct *l, struct Thread *threading )
{
  ...
  int start, end;
  compute_core_start_end(l->p_double.block_jacobi_double.local_p.v_start, l->p_double.block_jacobi_double.local_p.v_end, &start, &end, l, threading);

  local_gmres_double_struct *p = &( l->p_double.block_jacobi_double.local_p );

  int d_poly = p->polyprec_double.d_poly;
  vector_double accum_prod = p->polyprec_double.accum_prod;
  vector_double product = p->polyprec_double.product;
  vector_double temp = p->polyprec_double.temp;
  vector_double lejas = p->polyprec_double.lejas;
