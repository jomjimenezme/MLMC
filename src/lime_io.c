...
void lime_check_info(lime_fileinfo lime, n_uint64_t data_size_per_site) {
  ...
  *((n_uint64_t)(lime.precision==64?sizeof(complex_double):sizeof(complex_double))));
}
...
void lime_read_conf( double *input_data, char *input_name, double *conf_plaq ) {
  ...
  *gl=g.global_lattice[0], *ll=g.local_lattice[0], read_size = 4*18*ll[X], precision=64;
  ...
  if ( g.my_rank == 0 ) {
    ...
    if ( precision==32 ) {
      MALLOC( double_buffer, double, read_size );
    }
  ...
  if ( g.my_rank == 0 ) {
    if (precision==64) {
      ...
      ASSERT( fread( double_buffer, sizeof(double), read_size, fin ) > 0 );
      for ( int i=0; i<read_size; i++ ) {
        byteswap( (char *) (double_buffer+i) );
        buffer_pt->data[i] = double_buffer[i];
      }
    }
  }
    ...
    for ( z=0; z<gl[Z]; z++ )
      for ( y=0; y<gl[Y]; y++ )
          ...
          if ( g.my_rank == 0 ) {
            ...
            if ( ! ( t == gl[T]-1 && z == gl[Z]-1 && y == gl[Y]-1 && x == gl[X]-ll[X]  ) ) {
              if (precision==64) {
                ...
                for ( i=0; i<read_size; i++ ) {
                  byteswap( (char *) (double_buffer+i) );
                  buffer_pt->next->data[i] =  double_buffer[i];
                }
              }
            }
          }
        ...
        if ( g.my_rank == 0 ) {
          ...
          if (precision==32)
            FREE( double_buffer, double, read_size );
        }
        #else
...
void lime_read_vector( double *phi, char *filename ) {
  ...
  if ( g.my_rank == 0 ) {
    ...
    if ( precision==32 )
      MALLOC( double_buffer, double, bar_size );
  ...
  if ( g.my_rank == 0 ) {
    if (precision==64) {
      ...
      ASSERT( fread( double_buffer, sizeof(double), bar_size, file ) > 0 );
    ...
    for ( z=0; z<gl[Z]; z++ )
      for ( y=0; y<gl[Y]; y++ )
        ...
        if ( g.my_rank == 0 ) {
          ...
          if (precision==32)
            FREE( double_buffer, double, bar_size );
        }
