...
//                               g.tol, _RIGHT, vcycle_double, &(g.p_MP), l );
      g.p.op = &(g.op_double);
...
void read_geometry_data( FILE *in, int ls ) {
  ...
  for ( i=0; i<ls; i++ ) {
    ...
    save_pt = &(g.trace_deflation_type[i]); g.trace_deflation_type[i] = 2;
    read_parameter( &save_pt, inputstr, "%d", 1, in, _DEFAULT_SET );
...
void validate_parameters( int ls, level_struct *l ) {
  ...
  for ( i=0; i<g.num_levels-1; i++ )
    for ( mu=0; mu<4; mu++) {
      ...
      ASSERT( g.num_eig_vect[i] % SIMD_LENGTH_double == 0 );
#endif
    }
