...
complex_double global_inner_product_double( vector_double phi, vector_double psi, int start, int end, level_struct *l, struct Thread *threading ) {
  ...
  VECTOR_FOR( int i=thread_start, i<thread_end, local_alpha += conj_double(phi[i])*psi[i], i++, l );
...
complex_double process_inner_product_double( vector_double phi, vector_double psi, int start, int end, level_struct *l, struct Thread *threading ) {
  ...
  THREADED_VECTOR_FOR( i, start, end, local_alpha += conj_double(phi[i])*psi[i], i++, l, threading );
...
complex_double local_xy_over_xx_double( vector_double phi, vector_double psi, int start, int end, level_struct *l  ) {
  ...
  complex_double numerator = 0.0; double denominator = 0.0;
  
  VECTOR_FOR( int i=start, i<end, numerator += conj_double(phi[i])*psi[i]; denominator += NORM_SQUARE_double(phi[i]), i++, l );
  
  if ( abs_double(denominator) < EPS_double ) {
    return 0.0;
  }
...
double process_norm_double( vector_double x, int start, int end, level_struct *l, struct Thread *threading ) {
  ...
  THREADED_VECTOR_FOR( i, start, end, local_alpha += NORM_SQUARE_double(x[i]), i++, l, threading );
...
void vector_double_plus( vector_double z, vector_double x, vector_double y, int start, int end, level_struct *l ) {
...
void vector_double_minus( vector_double z, vector_double x, vector_double y, int start, int end, level_struct *l ) {
...
void vector_double_real_scale( vector_double z, vector_double x, complex_double alpha,
  ...
  double *r_z = (double*)z, *r_x = (double*)x, r_alpha = creal_double(alpha);
...
void vector_double_copy( vector_double z, vector_double x, int start, int end, level_struct *l ) {
...
void vector_double_multi_saxpy( vector_double z, vector_double *V, complex_double *alpha,
  ...
  complex_double alpha_signed[count];
  for ( int c=0; c<count; c++ ) {
...
void vector_double_projection( vector_double z, vector_double v, int k, vector_double *W, complex_double *diag, 
  ...
  vector_double v_tmp = NULL, *W_tmp = NULL;
  complex_double ip[k], ip_buffer[2*k];      
  
  MALLOC( v_tmp, complex_double, l->inner_vector_size );
  vector_double_define(v_tmp, 0, 0, l->inner_vector_size, l );
  
  MALLOC( W_tmp, complex_double*, k );
  W_tmp[0] = NULL; 
  MALLOC( W_tmp[0], complex_double, k*l->inner_vector_size );
  for ( j = 1; j<k; j++ )
    W_tmp[j] = W_tmp[0]+j*l->inner_vector_size;
  
  for ( j=0; j<k; j++ ) {
   vector_double_scale( W_tmp[j], W[j], diag[j], 0, l->inner_vector_size, l );
  }
  process_multi_inner_product_double( k, ip, W_tmp, v, 0, l->inner_vector_size, l, threading );
  
  START_MASTER(threading)
  for ( j=0; j<k; j++ ) {
    ip_buffer[j] = ip[j];
  }
  MPI_Allreduce( ip_buffer, ip_buffer+k, k, MPI_COMPLEX_double, MPI_SUM, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
  END_MASTER(threading)
  SYNC_MASTER_TO_ALL(threading)  
  
  vector_double_multi_saxpy( v_tmp, W_tmp, ip_buffer+k, 1, k, 0, l->inner_vector_size, l );
   
  if (orthogonal) 
    vector_double_minus( z, v, v_tmp, 0, l->inner_vector_size, l );
  else
    vector_double_copy( z, v_tmp, 0, l->inner_vector_size, l );
  
  FREE( v_tmp, complex_double, l->inner_vector_size );
  FREE( W_tmp[0], complex_double, k*l->inner_vector_size );
  FREE( W_tmp, complex_double*, k );
}

void gram_schmidt_on_aggregates_double( vector_double *V, const int num_vect, level_struct *l, struct Thread *threading ) {
  ...
  for ( j=threading->n_thread*threading->core+threading->thread; j<num_aggregates; j+=threading->n_thread*threading->n_core ) {
    for ( k1=0; k1<num_vect; k1++ ) {
      ...
      for ( k2=0; k2<k1; k2++ ) {
        ...
        for ( i=0; i<aggregate_size; ) {
          for ( k=0; k<offset; k++, i++ )
            alpha1 += conj_double(v_pt2[i]) * v_pt1[i];
          for ( k=0; k<offset; k++, i++ )
            alpha2 += conj_double(v_pt2[i]) * v_pt1[i];
        }
        for ( i=0; i<aggregate_size; ) {
          for ( k=0; k<offset; k++, i++ )
      ...
      for ( i=0; i<aggregate_size; ) {
        for ( k=0; k<offset; k++, i++ )
          v_pt1[i] =  norm1 * creal_double(v_pt1[i]) + I*norm1* cimag_double(v_pt1[i]);
        for ( k=0; k<offset; k++, i++ )
          v_pt1[i] =  norm2 * creal_double(v_pt1[i]) + I*norm2* cimag_double(v_pt1[i]);
      }
    }
  }
  SYNC_HYPERTHREADS(threading)
...
void spinwise_double_skalarmultiply( vector_double eta1, vector_double eta2, vector_double phi, complex_double alpha,
                                        int start, int end, level_struct *l ) {
  
  PROF_double_START( _LA6 );  
  for ( int i=start; i<end; ) {
    FOR6( eta1[i] = alpha*phi[i]; eta2[i] = _COMPLEX_double_ZERO; i++; )
    FOR6( eta2[i] = alpha*phi[i]; eta1[i] = _COMPLEX_double_ZERO; i++; )
  }
...
void set_boundary_double( vector_double phi, complex_double alpha, level_struct *l, struct Thread *threading ) {
  ...
  PROF_double_START( _SET, threading );
  int i;
  ...
  SYNC_CORES(threading)
  PROF_double_STOP( _SET, (double)(l->vector_size-l->inner_vector_size)/(double)l->inner_vector_size, threading );
}


void gram_schmidt_double( vector_double *V, complex_double *buffer, const int begin, const int n, level_struct *l, struct Thread *threading ) {
  ...
  for ( i=begin; i<n; i++ ) {
    ...
    complex_double tmp[i];
    process_multi_inner_product_double( i, tmp, V, V[i], 0, l->inner_vector_size, l, threading );
    SYNC_CORES(threading)
    START_MASTER(threading)
    for ( j=0; j<i; j++ ) {
      buffer[j] = tmp[j];
    ...
    if ( i>0 ) {
      START_MASTER(threading)
      PROF_double_START( _ALLR );
      MPI_Allreduce( buffer, buffer+n, i, MPI_COMPLEX_double, MPI_SUM, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
      PROF_double_STOP( _ALLR, 1 );
      END_MASTER(threading)
      SYNC_MASTER_TO_ALL(threading)
    }
    ...
    beta = global_norm_double( V[i], 0, l->inner_vector_size, l, threading );
    SYNC_MASTER_TO_ALL(threading)
    vector_double_real_scale( V[i], V[i], creal(1.0/beta), start, end, l );
    SYNC_CORES(threading)
  }
...
void setup_gram_schmidt_double_compute_dots(
  ...
  complex_double tmp[cache_block_size];
  ...
  for ( int i=thread_start; i<thread_end; i+=cache_block_size) {
    ...
    for ( int j=0; j<count; j++ ) {
      for ( int k=0; k<cache_block_size; k++) {
        thread_buffer[j]   += conj_double(V[j][i+k])*V[count][i+k];
        thread_buffer[j+offset] += conj_double(V[j][i+k])*tmp[k];
      }
    }
  }

  START_NO_HYPERTHREADS(threading)
  ((complex_double **)threading->workspace)[threading->core] = thread_buffer;
  END_NO_HYPERTHREADS(threading)
  // master sums up all results
  SYNC_CORES(threading)
  START_MASTER(threading)
  for(int i=1; i<threading->n_core; i++) {
    for(int j=0; j<count; j++) {
      ((complex_double **)threading->workspace)[0][j]        += ((complex_double **)threading->workspace)[i][j];
      ((complex_double **)threading->workspace)[0][j+offset] += ((complex_double **)threading->workspace)[i][j+offset];
    }
  }
  END_MASTER(threading)
...
void setup_gram_schmidt_double( vector_double *V, vector_double g5v,
  ...
  PROF_double_START( _GRAM_SCHMIDT, threading );
  double beta;
  int i, j;
  int start = 0;
  ...
  complex_double thread_buffer[4*n];
  ...
  for ( i=0; i<n; i++ ) {
    ...
    if ( l->depth > 0 ) {
      coarse_gamma5_double( g5v, V[i], thread_start, thread_end, l );
      for ( j=0; j<i; j++ ) {
        thread_buffer[j] = process_inner_product_double( V[j], V[i], start, end, l, threading );
        thread_buffer[j+n] = process_inner_product_double( V[j], g5v, start, end, l, threading );
      }
    }
    else
      setup_gram_schmidt_double_compute_dots( thread_buffer, V, i, n, start, end, l, threading);
    
    
    START_LOCKED_MASTER(threading)
    if ( i>0 ) {
      PROF_double_START( _ALLR );
      MPI_Allreduce( thread_buffer, thread_buffer+2*n, 2*n, MPI_COMPLEX_double, MPI_SUM, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
      PROF_double_STOP( _ALLR, 1 );
    }
    for ( j=2*n; j<4*n; j++ )
      ((complex_double *)(threading->workspace))[j] = thread_buffer[j];
    END_LOCKED_MASTER(threading)
    for ( j=2*n; j<4*n; j++ )
      thread_buffer[j] = ((complex_double *)(threading->workspace))[j];

    
    if ( l->depth > 0 ) {
      for( j=0; j<i; j++ ) {
        vector_double_saxpy( V[i], V[i], V[j], -(thread_buffer+2*n)[j], thread_start, thread_end, l );
        coarse_gamma5_double( g5v, V[j], thread_start, thread_end, l );
        vector_double_saxpy( V[i], V[i], g5v, -(thread_buffer+3*n)[j], thread_start, thread_end, l );
      }
    } else {
      setup_gram_schmidt_double_axpys( thread_buffer, V, i, n, start, end, l, threading);
    }
    
    beta = global_norm_double( V[i], start, end, l, threading );
    vector_double_real_scale( V[i], V[i], 1.0/beta, thread_start, thread_end, l );
  }
  PROF_double_STOP( _GRAM_SCHMIDT, (double)(end-start)/(double)l->inner_vector_size, threading );
}
