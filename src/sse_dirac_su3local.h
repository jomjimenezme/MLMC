...
#define CURRENT_MADD cfmadd
      ...
      {
#ifdef BOUNDARY
        double *pt = phi+48*ind[i+1];
#else
#ifdef MINUSDIR
        double *pt = phi+48*ind[i];
#else
        double *pt = phi+48*neighbor[4*ind[i]+MU];
#endif
#endif
        ...
        { // perform su(3) matrix vector multiplication
          ...
          {          
            ...
            __m128 buf2 = _mm_loadu_ps( pt+SIMD_LENGTH_double );
            {
          ...
          {
            __m128 buf1 = _mm_loadu_ps( pt+2*SIMD_LENGTH_double );
            __m128 buf2 = _mm_loadu_ps( pt+3*SIMD_LENGTH_double );
            {
          ...
          {
            __m128 buf1 = _mm_loadu_ps( pt+4*SIMD_LENGTH_double );
            __m128 buf2 = _mm_loadu_ps( pt+5*SIMD_LENGTH_double );
            {
      ...
      { // store result
#ifdef BOUNDARY
        double *pt = eta+48*ind[i];
#else
#ifdef MINUSDIR
        double *pt = eta+48*neighbor[4*ind[i]+MU];
#else
        double *pt = eta+48*ind[i];
#endif
#endif
...
#define CURRENT_MADD cfmadd
    ...
    {
#ifdef BOUNDARY
      double *pt = phi+24*ind[i+1];
#else
#ifdef MINUSDIR
      double *pt = phi+24*ind[i];
#else
      double *pt = phi+24*neighbor[4*ind[i]+MU];
#endif
#endif
    ...
    { // store result
#ifdef BOUNDARY
      double *pt = eta+24*ind[i];
#else
#ifdef MINUSDIR
      double *pt = eta+24*neighbor[4*ind[i]+MU];
#else
      double *pt = eta+24*ind[i];
#endif
#endif
