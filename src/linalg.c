...
void process_multi_inner_product_MP( int count, complex_double *results, vector_double *phi,
  ...
  for(int c=0; c<count; c++) {
    for ( i=thread_start; i<thread_end; ) {
      FOR12( results[c] += (complex_double) conj_double(phi[c][i])*psi[i]; i++; )
    }
  }
...
double global_norm_MP( vector_double x, int start, int end, level_struct *l, struct Thread *threading ) {
  ...
  SYNC_CORES(threading)
  for ( i=thread_start; i<thread_end; )
    FOR12( local_alpha += (complex_double) NORM_SQUARE_double(x[i]); i++; )
