...
static inline void sse_complex_deinterleaved_load( double *data, __m128 *result_re, __m128 *result_im  ) {  
  *result_re = _mm_setr_ps( data[0], data[2], data[4], data[6] );
  *result_im = _mm_setr_ps( data[1], data[3], data[5], data[7] );
}


static inline void sse_complex_interleaved_store( __m128 data_re, __m128 data_im, double *result  ) { 
  _mm_storeu_ps( result,                   _mm_unpacklo_ps( data_re, data_im ) );
  _mm_storeu_ps( result+SIMD_LENGTH_double, _mm_unpackhi_ps( data_re, data_im ) );
}
