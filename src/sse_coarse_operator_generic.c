...
void coarse_operator_double_setup_vectorized( complex_double *operator, level_struct *l, struct Thread *threading ) {
  ...
  int mu, j, n = l->num_eig_vect, num_aggregates = l->is_double.num_agg,
  ...
  for ( int a=threading->n_core*threading->thread+threading->core; a<num_aggregates; a+=threading->n_core*threading->n_thread ) {
    for ( j=0; j<D_link_size; j++ )
      l->next_level->op_double.D[j+a*D_link_size] = _COMPLEX_double_ZERO;
    for ( j=0; j<clover_site_size; j++ )
      l->next_level->op_double.clover[j+a*clover_site_size] = _COMPLEX_double_ZERO;
    for ( j=0; j<block_site_size; j++ )
      l->next_level->op_double.odd_proj[j+a*block_site_size] = _COMPLEX_double_ZERO;
  }

  complex_double *mpi_buffer = NULL;
  START_MASTER(threading)
  MALLOC_HUGEPAGES( mpi_buffer, complex_double, OPERATOR_COMPONENT_OFFSET_double*(l->vector_size-l->inner_vector_size), 64 );
  ...
  complex_double eta1[fine_components*OPERATOR_COMPONENT_OFFSET_double] __attribute__((aligned(64)));
  complex_double eta2[fine_components*OPERATOR_COMPONENT_OFFSET_double] __attribute__((aligned(64)));
  complex_double tmp[4*4*n*OPERATOR_COMPONENT_OFFSET_double] __attribute__((aligned(64)));

  for ( int a=threading->n_core*threading->thread+threading->core; a<num_aggregates; a+=threading->n_core*threading->n_thread ) {

    // new aggregate is starting, zero out tmp
    for(int i=0; i<4*4*n*OPERATOR_COMPONENT_OFFSET_double; i++)
      tmp[i] = 0.0;

    for ( int site=a*aggregate_sites; site<(a+1)*aggregate_sites; site++ ) {
      if(l->depth == 0) {
        for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
          d_plus_clover_aggregate_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
              operator+c*l->vector_size, &(l->s_double), l, site,
              direction_flags+8*(site%(l->block_lattice[T]*l->block_lattice[Z]*l->block_lattice[Y]*l->block_lattice[X])) );
      } else {
        for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
          coarse_aggregate_self_couplings_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
              operator+c*l->vector_size, &(l->s_double), l, site,
              direction_flags+8*(site%(l->block_lattice[T]*l->block_lattice[Z]*l->block_lattice[Y]*l->block_lattice[X])) );
      }
      set_coarse_self_coupling_double_vectorized( eta1, eta2, operator, l, site, n, tmp );
    }

    // aggregate is done, finalize
    set_coarse_self_coupling_double_vectorized_finalize( l, a*aggregate_sites, n, tmp );
  ...
  for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double ) {
    for ( mu=0; mu<4; mu++ ) {
      ...
      int start = 0;
      for ( int j=0; j<mu; j++ )
        start += l->s_double.op.c.num_boundary_sites[2*j];

      // update ghost cells of V[i]
      negative_sendrecv_double_vectorized( operator+c*l->vector_size, mu, &(l->s_double.op.c), l,
          SIMD_LENGTH_double, mpi_buffer+c*(l->vector_size-l->inner_vector_size)+fine_components*start*SIMD_LENGTH_double );
    }
    for ( mu=0; mu<4; mu++ ) {
  ...
  for ( int a=threading->n_core*threading->thread+threading->core; a<num_aggregates; a+=threading->n_core*threading->n_thread ) {
    ...
    for ( int site=a*aggregate_sites; site<(a+1)*aggregate_sites; site++ ) {
      for ( mu=0; mu<4; mu++ ) {
        ...
        if(l->depth == 0)
          for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
            d_neighbor_aggregate_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
                operator+c*l->vector_size, mu, &(l->s_double), l, site );
        else
          for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
            coarse_aggregate_neighbor_couplings_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
                operator+c*l->vector_size, mu, &(l->s_double), l, site );
        set_coarse_neighbor_coupling_double_vectorized( eta1, eta2, operator, mu, l, site, n, tmp+mu*4*n*OPERATOR_COMPONENT_OFFSET_double );
      }
    }

    // aggregate is done, finalize
    for ( mu=0; mu<4; mu++ )
      set_coarse_neighbor_coupling_double_vectorized_finalize( mu, l, a*aggregate_sites, n, tmp+mu*4*n*OPERATOR_COMPONENT_OFFSET_double );
  }
  ...
  for ( int a=threading->n_core*threading->thread+threading->core; a<num_aggregates; a+=threading->n_core*threading->n_thread ) {
    ...
    for ( int site=a*aggregate_sites; site<(a+1)*aggregate_sites; site++ ) {
      if(l->depth == 0) {
        for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
          diagonal_aggregate_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
                                                   operator+c*l->vector_size, &(l->s_double), l, site );
      } else {
        for ( int c=0; c<l->num_eig_vect; c+=SIMD_LENGTH_double )
          coarse_aggregate_block_diagonal_double_vectorized( eta1+c*fine_components, eta2+c*fine_components,
              operator+c*l->vector_size, &(l->s_double), l, site );
      }
      set_coarse_block_diagonal_double_vectorized( eta1, eta2, operator, l, site, n, tmp );
    }

    // aggregate is done, finalize
    set_coarse_block_diagonal_double_vectorized_finalize( l, a*aggregate_sites, n, tmp );
  }

  SYNC_HYPERTHREADS(threading)
  SYNC_CORES(threading)

  coarse_operator_double_setup_finalize( l, threading );

  START_MASTER(threading)
  FREE_HUGEPAGES( mpi_buffer, complex_double, OPERATOR_COMPONENT_OFFSET_double*(l->vector_size-l->inner_vector_size) );
...
void set_coarse_self_coupling_double_vectorized_finalize( level_struct *l, int site, const int n_rhs, complex_double *tmp ) {
  ...
  int component_offset = OPERATOR_COMPONENT_OFFSET_double;
  ...
  for ( int n=0; n<n_rhs; n++ ) {
    ...
    for ( k=0; k<=n; k++ ) {
      ...
      // A
      clover_pt[ k1+k ] += ((double *)(tmp+t1))[k] + I * ((double *)(tmp+t1)+component_offset)[k];

      // D
      clover_pt[ k2+k ] += ((double *)(tmp+t2))[k] + I * ((double *)(tmp+t2)+component_offset)[k];
    }
...
void set_coarse_neighbor_coupling_double_vectorized( complex_double *spin_0_1, complex_double *spin_2_3, 
                                                        complex_double *V, const int mu, level_struct *l, int site,
                                                        const int n_rhs, complex_double *tmp ) {

  sse_set_coarse_neighbor_coupling_double( spin_0_1, spin_2_3, V, mu, l, site, n_rhs, tmp );
}


void set_coarse_neighbor_coupling_double_vectorized_finalize( const int mu, level_struct *l, int site,
                                                                 const int n_rhs, complex_double *tmp ) {
  ...
  int aggregate = (fine_components*site)/(l->inner_vector_size / l->is_double.num_agg);
  ...
  for ( int n=0; n<n_rhs; n++ ) {
    ...
    for ( k=0; k<num_eig_vect; k++ ) {
      ...
      // A
      D_pt[ k1+k ] += ((double *)(tmp+t1))[k] + I * ((double *)(tmp+t1)+component_offset)[k];

      // C
      D_pt[ k2+k ] += ((double *)(tmp+t2))[k] + I * ((double *)(tmp+t2)+component_offset)[k];
...
void set_coarse_block_diagonal_double_vectorized_finalize( level_struct *l, int site, const int n_rhs, complex_double *tmp ) {
  ...
  config_double block_pt, block = l->next_level->op_double.odd_proj;
  ...
  for ( int n=0; n<n_rhs; n++ ) {
    ...
    for ( k=0; k<=n; k++ ) {
      ...
      // A
      block_pt[ k1+k ] += ((double *)(tmp+t1))[k] + I * ((double *)(tmp+t1)+component_offset)[k];

      // D
      block_pt[ k2+k ] += ((double *)(tmp+t2))[k] + I * ((double *)(tmp+t2)+component_offset)[k];
    }
  }
}
...
void copy_coarse_operator_to_vectorized_layout_double( config_double D,
  ...
  int half_offset = SIMD_LENGTH_double*((vecs+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  int column_offset = 2*half_offset;
  // offset between blocks in D
  int block_offset = vecs*vecs;

  double *out_tmp = D_vectorized;

  // we zero out the padded area (o) to avoid potential doubleing-point errors
  // D_vectorized is
  // AB
  // oo
  // CD
...
void copy_coarse_operator_to_transformed_vectorized_layout_double(config_double D,
  ...
  // we zero out the padded area to avoid potential doubleing-point errors
  // D_vectorized is
  // A^T C^T
  //  o   o
  // B^T D^T
...
void copy_coarse_operator_clover_to_vectorized_layout_double(config_double clover,
  ...
  // each column is padded, such that next column can also start at 64B boundary
  int column_offset = SIMD_LENGTH_double*((2*num_eig_vect+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  ...
  double *out_tmp = clover_vectorized;
...
void copy_coarse_operator_clover_to_doublet_vectorized_layout_double(config_double clover,
  ...
  // in vectorized layout clover is stored column wise, but not split into ABCD
  // each column is padded, such that next column can also start at 64B boundary
  int column_offset = SIMD_LENGTH_double*((4*vecs+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  // offset between blocks in clover
  int offset_to_D = (vecs*vecs+vecs)/2; // upper triangle of A including diagonal
  int offset_to_B = 2*offset_to_D; // B comes after A and D
  ...
  // C0D0
  // 0C0D
  // 0000  we zero out the padded area to avoid potential doubleing-point errors
  // (column wise, size of zeros such that columns length is multiple of 64B)
...
void add_tm_term_to_vectorized_layout_double(config_double tm_term, OPERATOR_TYPE_double *clover_vectorized,
                                                int num_aggregates, int num_eig_vect) {
#ifdef HAVE_TM
  int vecs = num_eig_vect;
  // in vectorized layout clover is stored column wise, but not split into ABCD
...
void add_tm_term_to_doublet_vectorized_layout_double(config_double tm_term, OPERATOR_TYPE_double *clover_vectorized,
                                                int num_aggregates, int num_eig_vect) {
#ifdef HAVE_TM
  int vecs = num_eig_vect;
  // in vectorized layout clover is stored column wise, but not split into ABCD
...
void add_epsbar_term_to_doublet_vectorized_layout_double(config_double eps_term, OPERATOR_TYPE_double *clover_vectorized,
                                                            int num_aggregates, int num_eig_vect) {
#ifdef HAVE_TM1p1
  int vecs = num_eig_vect;
  // in vectorized layout clover is stored column wise, but not split into ABCD
...
void coarse_aggregate_self_couplings_double_vectorized( complex_double *eta1, complex_double *eta2, 
  ...
  coarse_spinwise_site_self_couplings_double_vectorized( eta1, eta2, phi+site_offset*site, s->op.clover+clover_offset, offset, l );
  ...
  for(int mu=0; mu<4; mu++) {
    ...
    if ( direction_flags[2*mu+0] == 1 ) {
      ...
      coarse_spinwise_n_daggered_hopp_double_vectorized( eta1, eta2, phi_pt, D_pt, offset, l );
    }
    ...
    if ( direction_flags[2*mu+1] == 1 ) {
      ...
      coarse_spinwise_n_hopp_double_vectorized( eta1, eta2, phi_pt, D_pt, offset, l );
    }
  }
}
...
void coarse_aggregate_block_diagonal_double_vectorized( complex_double *eta1, complex_double *eta2, 
  ...
  sse_coarse_aggregate_block_diagonal_double( eta1, eta2, phi+site_offset*site, s->op.odd_proj+block_offset, offset, l );
}

void coarse_aggregate_neighbor_couplings_double_vectorized( complex_double *eta1, complex_double *eta2, 
                                                               complex_double *phi, const int mu,
                                                               schwarz_double_struct *s, level_struct *l, int site ) {
  ...
  vector_double_define( eta1, 0, 0, n*offset, l );
  vector_double_define( eta2, 0, 0, n*offset, l );

  // requires the positive boundaries of phi to be communicated before
  index_fw  = neighbor[5*site+1 + mu];
  D_pt = D + D_site_offset*site + D_link_offset*mu;
  phi_pt = phi + site_offset*index_fw;
  coarse_spinwise_hopp_double_vectorized( eta1, eta2, phi_pt, D_pt, offset, l );
}
...
void coarse_spinwise_site_self_couplings_double_vectorized(
  ...
  sse_coarse_spinwise_site_self_couplings_double( eta1, eta2, phi, clover, elements, l );
}
