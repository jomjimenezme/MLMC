...
void pgeqr2_double( int*, int*, MKL_double*, int*, int*, int*, MKL_double*, MKL_double*, int*, int* );
void pung2r_double( int*, int*, int*, MKL_double*, int*, int*, int*, MKL_double*, MKL_double*, int*, int* );
...
void inv_tri_double(eigslvr_double_struct* eigen_struct)
{
  ...
  trtri_double( LAPACK_COL_MAJOR, 'U', 'N', eigen_struct->qr_k, eigen_struct->qr_R[0],
                   eigen_struct->qr_k);
}
...
void qr_double(eigslvr_double_struct* eigen_struct)
{
  ...
  geqr2_double( LAPACK_COL_MAJOR, eigen_struct->qr_m, eigen_struct->qr_n, eigen_struct->qr_QR[0],
...
void q_from_qr_double(eigslvr_double_struct* eigen_struct)
{
  ...
  ungqr_double( LAPACK_COL_MAJOR, eigen_struct->qr_m, eigen_struct->qr_n, eigen_struct->qr_k,
...
void gen_eigslvr_double(eigslvr_double_struct* eigen_struct)
{
  ...
  eigen_struct->info = ggev_double( LAPACK_COL_MAJOR, eigen_struct->jobvl, eigen_struct->jobvr,
...
void eigslvr_double(eigslvr_double_struct* eigen_struct)
{
  ...
  eigen_struct->info = geev_double( LAPACK_ROW_MAJOR, eigen_struct->jobvl, eigen_struct->jobvr,
...
void dirctslvr_double(dirctslvr_double_struct* dirctslvr)
{
    ...
    memcpy( dirctslvr->x, dirctslvr->b, sizeof(complex_double)*(dirctslvr->N) );  

    dirctslvr->info = gesv_double( LAPACK_COL_MAJOR, dirctslvr->N, dirctslvr->nrhs, dirctslvr->Hcc, 
...
void pqr_double( int mx, int nx, complex_double **Ax, complex_double **R, gmres_double_struct *p, level_struct *l, struct Thread *threading )
{
  ...
  for( j=0;j<k;j++ ) {
    ...
    if( j>0 ) {
      complex_double *bf = p->gcrodr_double.R[j];
      complex_double *buffer = p->gcrodr_double.Bbuff[0];
      complex_double tmpx[j];
      process_multi_inner_product_double( j, tmpx, Ax, Ax[j], p->v_start, p->v_end, l, threading );
      START_MASTER(threading)
      for ( int ww=0; ww<j; ww++ )
        buffer[ww] = tmpx[ww];
      if ( g.num_processes > 1 ) {
        PROF_double_START( _ALLR );
        MPI_Allreduce( buffer, bf, j, MPI_COMPLEX_double, MPI_SUM, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
      ...
      for( int ww=0; ww<j; ww++ ) {
        vector_double_saxpy( Ax[j], Ax[j], Ax[ww], -R[j][ww], start, end, l );
        SYNC_MASTER_TO_ALL(threading)
      }
    ...
    {
      double nrm_Ai = global_norm_double( Ax[j], p->v_start, p->v_end, l, threading );
      START_MASTER(threading)
      R[j][j] = nrm_Ai;
      END_MASTER(threading)
      SYNC_MASTER_TO_ALL(threading)
      vector_double_real_scale( Ax[j], Ax[j], 1/creal(R[j][j]), start, end, l );
      SYNC_MASTER_TO_ALL(threading)
    }
  ...
  complex_double *a = A[0];

  int izero=0;
  int ione=1;
  int myrank_mpi, nprocs_mpi;

  MPI_Comm_rank( (l->depth==0)?g.comm_cart:l->gs_double.level_comm, &myrank_mpi );
  MPI_Comm_size( (l->depth==0)?g.comm_cart:l->gs_double.level_comm, &nprocs_mpi );
  ...
  MALLOC( taux, complex_double, nx );
  MALLOC( work, complex_double, lwork );

  MPI_Barrier(MPI_COMM_WORLD);

  // compute QR in parallel
  lwork=-1;
  pgeqr2_double( (MKL_INT*) &mx, (MKL_INT*) &nx, (MKL_double*)a, (MKL_INT*) &ia,
                    (MKL_INT*) &ja, (MKL_INT*) descA, (MKL_double*)taux, (MKL_double*)work,
                    (MKL_INT*) &lwork, (MKL_INT*) &info );
  if (info != 0) {
    error0("Error in pgeqrf_double, info = %d\n", info);
  }
  ...
  if ( g.my_rank==0 ) {
    // copy/extract R
    memset( R[0], 0, sizeof(complex_double)*nx*nx );
    // compute R^{-1}
    for ( int j=0; j<nx; j++ ) {
      for ( int i=0; i<(j+1); i++ ) {
        R[j][i] = A[j][i];
    ...
    MPI_Bcast( R[0], nx*nx, MPI_COMPLEX_double, g.my_rank, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
  } else {
    MPI_Bcast( R[0], nx*nx, MPI_COMPLEX_double, 0, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
  }
  ...
  // construct Q from QR
  lwork=-1;
  pung2r_double( (MKL_INT*) &mx, (MKL_INT*) &nx, (MKL_INT*) &nx, (MKL_double*)a, (MKL_INT*) &ia,
                    (MKL_INT*) &ja, (MKL_INT*) descA, (MKL_double*)taux, (MKL_double*)work,
                    (MKL_INT*) &lwork, (MKL_INT*) &info );
  if (info != 0) {
    error0("Error in pungqr_double, info = %d\n", info);
  }

  FREE( taux, complex_double, nx );
  FREE( work, complex_double, lwork );
  if (info != 0) {
    error0("Error in pgeqrf_double, info = %d\n", info);
  }
...
// least squares
//void least_sq_double( eigslvr_double_struct* eigen_struct )
//{
