...
static inline void sse_set_coarse_self_coupling_double( complex_double *spin_0_1, complex_double *spin_2_3,
  ...
  double *spin_0_1_pt;
  double *spin_2_3_pt;
  double *interpolation_data;

  int component_offset = SIMD_LENGTH_double;
  ...
  for ( int n=0; n<n_rhs; n++ ) {
    ...
    int max = SIMD_LENGTH_double*((n+1+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
    spin_0_1_pt = (double *)(spin_0_1 + (n/SIMD_LENGTH_double)*2*offset*SIMD_LENGTH_double) + n%SIMD_LENGTH_double;
    spin_2_3_pt = (double *)(spin_2_3 + (n/SIMD_LENGTH_double)*2*offset*SIMD_LENGTH_double) + n%SIMD_LENGTH_double;


    // index k used for vectorization
    // original loop runs to k<=n, we must pad as usual to fill SIMD
    for ( k=0; k<max; k+=SIMD_LENGTH_double ) {
      __m128 buffer_re;
      __m128 buffer_im;
      ...
      k1 = (n+0*num_eig_vect)*OPERATOR_COMPONENT_OFFSET_double;
      k2 = (n+1*num_eig_vect)*OPERATOR_COMPONENT_OFFSET_double;

      interpolation_data = (double *)(V + k*l->vector_size + fine_components*SIMD_LENGTH_double*site);

      // A
      buffer_re = _mm_load_ps((double *)(tmp+k1)+k+0*OPERATOR_COMPONENT_OFFSET_double);
      buffer_im = _mm_load_ps((double *)(tmp+k1)+k+1*OPERATOR_COMPONENT_OFFSET_double);
      for ( m=0; m<offset; m++ ) {
      ...
      _mm_store_ps((double *)(tmp+k1)+k+0*OPERATOR_COMPONENT_OFFSET_double, buffer_re);
      _mm_store_ps((double *)(tmp+k1)+k+1*OPERATOR_COMPONENT_OFFSET_double, buffer_im);

      // D
      buffer_re = _mm_load_ps((double *)(tmp+k2)+k+0*OPERATOR_COMPONENT_OFFSET_double);
      buffer_im = _mm_load_ps((double *)(tmp+k2)+k+1*OPERATOR_COMPONENT_OFFSET_double);
      ...
      _mm_store_ps((double *)(tmp+k2)+k+0*OPERATOR_COMPONENT_OFFSET_double, buffer_re);
      _mm_store_ps((double *)(tmp+k2)+k+1*OPERATOR_COMPONENT_OFFSET_double, buffer_im);
    }

    // index k used for vectorization
    for ( k=0; k<OPERATOR_COMPONENT_OFFSET_double; k+=SIMD_LENGTH_double ) {
      __m128 buffer_re;
      __m128 buffer_im;

      // this are the packed indices, which we do not use in tmp
      //k1 = component_offset*(num_eig_vect+1+n);
      k1 = (n+2*num_eig_vect)*OPERATOR_COMPONENT_OFFSET_double;

      interpolation_data = (double *)(V + k*l->vector_size + fine_components*component_offset*site);
...
static inline void sse_set_coarse_neighbor_coupling_double( complex_double *spin_0_1, complex_double *spin_2_3,
  ...
  for ( int n=0; n<n_rhs; n++ ) {
    ...
    for ( k=0; k<OPERATOR_COMPONENT_OFFSET_double; k+=SIMD_LENGTH_double ) {
      ...
      k1 = (n+2*num_eig_vect)*OPERATOR_COMPONENT_OFFSET_double;
      k2 = (n+3*num_eig_vect)*OPERATOR_COMPONENT_OFFSET_double;
  ...
  for(int s=0; s<2; s++) {
    for(int i=0; i<elements; i+=SIMD_LENGTH_double) {
      for(int row=0; row<2*num_eig_vect; row++) {
        _mm_store_ps((double *)eta[s] + i + (2*row+0)*elements, zero);
        _mm_store_ps((double *)eta[s] + i + (2*row+1)*elements, zero);
      }
    }
  }
...
static inline void sse_coarse_aggregate_block_diagonal_double( complex_double *eta1, complex_double *eta2,
    complex_double *phi, config_double block, int elements, level_struct *l ) {
