...
void coarse_operator_double_free_vectorized( operator_double_struct *op, level_struct *l ) {
  ...
  if( op->D_vectorized != NULL ) {
    ...
    FREE_HUGEPAGES( op->D_vectorized, OPERATOR_TYPE_double, 2*4*2*l->num_parent_eig_vect*column_offset*n2 );
    FREE_HUGEPAGES( op->D_transformed_vectorized, OPERATOR_TYPE_double, 2*4*2*l->num_parent_eig_vect*column_offset*n2 );
  }
#endif
  ...
  if( op->clover_vectorized != NULL ) {
    ...
    int column_offset = SIMD_LENGTH_double*((2*l->num_parent_eig_vect+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
    FREE_HUGEPAGES( op->clover_vectorized, OPERATOR_TYPE_double, 2*2*l->num_parent_eig_vect*column_offset*n );
#ifdef HAVE_TM1p1
    int column_doublet_offset = SIMD_LENGTH_double*((4*l->num_parent_eig_vect+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
    FREE_HUGEPAGES( op->clover_doublet_vectorized, OPERATOR_TYPE_double, 2*4*l->num_parent_eig_vect*column_doublet_offset*n );
#endif
  }
#endif
}
...
void coarse_operator_double_setup( vector_double *V, level_struct *l ) {
  ...
  void (*aggregate_self_coupling)() = (l->depth==0)?d_plus_clover_aggregate_double:coarse_aggregate_self_couplings_double,
       (*aggregate_neighbor_coupling)() = (l->depth==0)?d_neighbor_aggregate_double:coarse_aggregate_neighbor_couplings_double;
  void (*aggregate_block)() = (l->depth==0)?diagonal_aggregate_double:coarse_aggregate_block_diagonal_double;
   
  operator_double_define( &(l->next_level->op_double), l->next_level );
    
  for ( j=0; j<D_size; j++ )
    l->next_level->op_double.D[j] = _COMPLEX_double_ZERO;
  for ( j=0; j<clover_size; j++ )
    l->next_level->op_double.clover[j] = _COMPLEX_double_ZERO;
  for ( j=0; j<block_size; j++ )
    l->next_level->op_double.odd_proj[j] = _COMPLEX_double_ZERO;
  ...
  coarse_operator_double_setup_finalize( l, no_threading );
...
#ifdef HAVE_TM    
  //tm_term
  double mf = (g.mu_factor[l->depth]) ? g.mu_factor[l->next_level->depth]/g.mu_factor[l->depth]:0;
  if ( mf*l->s_double.op.mu + mf*l->s_double.op.mu_even_shift == 0 &&
       mf*l->s_double.op.mu + mf*l->s_double.op.mu_odd_shift == 0 )
    vector_double_define( l->next_level->op_double.tm_term, _COMPLEX_double_ZERO, 0, block_size, l->next_level );
  else
...
#ifdef HAVE_TM1p1
  //eps_term
  double ef = (g.epsbar_factor[l->depth]) ? g.epsbar_factor[l->next_level->depth]/g.epsbar_factor[l->depth]:0; 
  if ( ef*l->s_double.op.epsbar == 0 &&  ef*l->s_double.op.epsbar_ig5_even_shift == 0 &&
       ef*l->s_double.op.epsbar_ig5_odd_shift == 0 )
    vector_double_define( l->next_level->op_double.epsbar_term, _COMPLEX_double_ZERO, 0, block_size, l->next_level );
  else
...
void set_block_diagonal_double( vector_double spin_0_1, vector_double spin_2_3, 
                                   vector_double *V, const int n, config_double block, level_struct *l ) {
  ...
  int i, j, k, m, k1, k2, num_aggregates = l->is_double.num_agg,
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_size; ) {
        ...
        for ( m=0; m<offset; m++, i++ )
          block_pt[ k1 ] += conj_double( interpolation_data[i] ) * spin_0_1_pt[i];
        // D
        for ( m=0; m<offset; m++, i++ )
          block_pt[ k2 ] += conj_double( interpolation_data[i] ) * spin_2_3_pt[i];
      }
    }
  }
}
  ...
  for ( k=0; k<=n; k++ ) {
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_size; ) {
        // A
        for ( m=0; m<offset; m++, i++ )
          clover_pt[ k1 ] += conj_double( interpolation_data[i] ) * spin_0_1_pt[i];
        // D
        for ( m=0; m<offset; m++, i++ )
          clover_pt[ k2 ] += conj_double( interpolation_data[i] ) * spin_2_3_pt[i];
      }
    }
  }
  ...
  for ( k=0; k<num_eig_vect; k++ ) {
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_size; ) {
        // B
        for ( m=0; m<offset; m++, i++ )
          clover_pt[ k1 ] += conj_double( interpolation_data[i] ) * spin_2_3_pt[i];
        i += offset;
      }
    }
  }
...
void set_coarse_neighbor_coupling_double( vector_double spin_0_1, vector_double spin_2_3, 
  ...
  int i, i1, j, k, k1, k2, m, num_aggregates = l->is_double.num_agg,
      num_eig_vect = l->next_level->num_parent_eig_vect,
      offset = l->num_parent_eig_vect, nlsv = l->num_parent_eig_vect*2,
      D_link_size = num_eig_vect*num_eig_vect*4, *index_dir = l->is_double.agg_boundary_index[mu],
  ...
  for ( k=0; k<num_eig_vect; k++ ) {
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_boundary_sites; i++ ) {
        ...
        // A
        for ( m=0; m<offset; m++ )
          D_pt[ k1 ] += conj_double( interpolation_data[m] ) * spin_0_1_pt[m];
        // C
        for ( ; m<2*offset; m++ )
          D_pt[ k2 ] += conj_double( interpolation_data[m] ) * spin_0_1_pt[m];
      }
    }
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_boundary_sites; i++ ) {
        ...
        // B
        for ( m=0; m<offset; m++ )
          D_pt[ k1 ] += conj_double( interpolation_data[m] ) * spin_2_3_pt[m];
        // D
        for ( ; m<2*offset; m++ )
          D_pt[ k2 ] += conj_double( interpolation_data[m] ) * spin_2_3_pt[m];
      }
    }
  }
}

void coarse_block_operator_double( vector_double eta, vector_double phi, int start, schwarz_double_struct *s, level_struct *l, struct Thread *threading ) {
...
#else
  int column_offset = 2*SIMD_LENGTH_double*((num_eig_vect+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  int vectorized_link_offset = 2*2*num_eig_vect*column_offset;
  for ( int mu=0; mu<4; mu++ ) {
    OPERATOR_TYPE_double *Dplus = s->op.D_vectorized +
    ...
    for ( int i=0; i<length[mu]; i++ ) {
      int k = ind[i]; int j = neighbor[5*k+mu+1];
      // hopp
      coarse_hopp_double_vectorized( leta+m*k, lphi+m*j, Dplus + 4*vectorized_link_offset*k, l );
      // daggered hopp
      coarse_hopp_double_vectorized( leta+m*j, lphi+m*k, Dminus + 4*vectorized_link_offset*k, l );
    }
  }
#endif
...
void coarse_aggregate_self_couplings_double( vector_double eta1, vector_double eta2, vector_double phi,
  ...
  vector_double_define( eta1, 0, 0, l->vector_size, l );
  vector_double_define( eta2, 0, 0, l->vector_size, l );  
  coarse_spinwise_self_couplings_double( eta1, eta2, phi, s->op.clover, l->inner_vector_size, l );
  
  for ( mu=0; mu<4; mu++ ) { // direction mu
    length = l->is_double.agg_length[mu]; index_dir = l->is_double.agg_index[mu];
    for ( i=0; i<length; i++ ) {
...
void coarse_aggregate_neighbor_couplings_double( vector_double eta1, vector_double eta2, vector_double phi,
                                                    const int mu, schwarz_double_struct *s, level_struct *l ) {
...
void coarse_aggregate_block_diagonal_double( vector_double eta1, vector_double eta2, vector_double phi,
                                                config_double block, level_struct *l ) {
  ...
  while ( phi_pt < phi_end_pt ) {
    ...
    vector_double_define( eta2_pt, _COMPLEX_double_ZERO, 0, num_eig_vect, l );
    block_pt += block_step_size; eta1_pt += num_eig_vect; eta2_pt += num_eig_vect; phi_pt += num_eig_vect;
    // D
    vector_double_define( eta1_pt, _COMPLEX_double_ZERO, 0, num_eig_vect, l );
...
void coarse_spinwise_self_couplings_double( vector_double eta1, vector_double eta2, vector_double phi,
  ...
  vector_double phi_pt=phi, eta1_pt=eta1, eta2_pt=eta2+num_eig_vect, phi_end_pt=phi+length;
  // U(x) = [ A B      , A=A*, D=D*, C = -B*
  //          C D ]
  // storage order: upper triangle of A, upper triangle of D, B, columnwise
  // diagonal coupling
...
void coarse_operator_double_set_couplings( operator_double_struct *op, level_struct *l,
                                              struct Thread *threading ) {

  coarse_operator_double_set_neighbor_couplings( op, l, threading );
  coarse_operator_double_set_self_couplings( op, l, threading );

}

void coarse_operator_double_set_neighbor_couplings( operator_double_struct *op, level_struct *l,
                                                       struct Thread *threading ) {
  ...
  copy_coarse_operator_to_vectorized_layout_double(
...
void coarse_operator_double_set_self_couplings( operator_double_struct *op, level_struct *l, 
                                                   struct Thread *threading ) {
...
#endif
    {
      warning0("coarse_tau1_gamma5_double called with g.n_flavours != 2\n");
      coarse_gamma5_double( eta, phi, start, end, l );
    }
}

void apply_coarse_operator_double( vector_double eta, vector_double phi, operator_double_struct *op,
...
#ifndef OPTIMIZED_COARSE_NEIGHBOR_COUPLING_double
  coarse_hopping_term_double( eta, phi, op, _FULL_SYSTEM, l, threading );
#else
  coarse_hopping_term_double_vectorized( eta, phi, op, _FULL_SYSTEM, l, threading ); 
#endif
...
void g5D_apply_coarse_operator_double( vector_double eta, vector_double phi, operator_double_struct *op,
  ...
  apply_coarse_operator_double( eta, phi, op, l, threading );
  SYNC_CORES(threading)
  coarse_gamma5_double( eta, eta, start, end, l );
  SYNC_CORES(threading)
}


void apply_coarse_operator_dagger_double( vector_double eta, vector_double phi, operator_double_struct *op,
                                             level_struct *l, struct Thread *threading ) {
  
  coarse_gamma5_double( l->vbuf_double[3], phi, threading->start_index[l->depth], threading->end_index[l->depth], l );
  apply_coarse_operator_double( eta, l->vbuf_double[3], op, l, threading );
  coarse_gamma5_double( eta, eta, threading->start_index[l->depth], threading->end_index[l->depth], l );
}
...
void coarse_operator_double_test_routine( level_struct *l, struct Thread *threading ) {
  ...
  if ( !l->idle ) {
    ...
    double diff = 0;
    vector_double vp1=NULL, vp2, vp3, vp4, vc1=NULL, vc2, vc3;

    PUBLIC_MALLOC( vp1, complex_double, 4*vs );
    PUBLIC_MALLOC( vc1, complex_double, 3*cvs );
...
#endif
    {
#ifdef INTERPOLATION_OPERATOR_LAYOUT_OPTIMIZED_double
      double norm = 0.0;
      double dot = 0.0;
      double *op = (double *)l->is_double.operator;
      double *op2 = (double *)(l->is_double.operator+0*SIMD_LENGTH_double*l->vector_size)+1;
      for ( int i=0; i<l->inner_vector_size; i++ )
        norm += (op[2*i*SIMD_LENGTH_double+0] + I*op[2*i*SIMD_LENGTH_double+SIMD_LENGTH_double])*conj(op[2*i*SIMD_LENGTH_double+0] + I*op[2*i*SIMD_LENGTH_double+SIMD_LENGTH_double]);
      for ( int i=0; i<l->inner_vector_size; i++ )
        dot += (op[2*i*SIMD_LENGTH_double+0] + I*op[2*i*SIMD_LENGTH_double+SIMD_LENGTH_double])*conj(op2[2*i*SIMD_LENGTH_double+0] + I*op2[2*i*SIMD_LENGTH_double+SIMD_LENGTH_double]);
      diff = dot/norm;
#else
      diff = global_inner_product_double( l->is_double.interpolation[0], l->is_double.interpolation[1], 0, ivs, l, no_threading )
        / global_norm_double( l->is_double.interpolation[0], 0, ivs, l, no_threading );
#endif
      test0_double("depth: %d, correctness of block_gram_schmidt: %le\n", l->depth, cabs(diff) );
    }
    
    if ( !l->next_level->idle )
      vector_double_define_random( vc1, 0, civs, l->next_level );
    vector_double_distribute( vc2, vc1, l->next_level );
    vector_double_gather( vc3, vc2, l->next_level );
    if ( !l->next_level->idle ) {
      vector_double_minus( vc2, vc1, vc3, 0, civs, l->next_level );
      diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
    }
    test0_double("depth: %d, correctness of gather( distribute( phi_c ) ) : %le\n", l->depth, diff );
        
    if ( !l->next_level->idle )
      vector_double_define_random( vc1, 0, civs, l->next_level );
    interpolate3_double( vp1, vc1, l, no_threading );
    restrict_double( vc2, vp1, l, no_threading );
    if ( !l->next_level->idle ) {
      vector_double_minus( vc3, vc1, vc2, 0, civs, l->next_level );
      diff = global_norm_double( vc3, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
      test0_double("depth: %d, correctness of ( P* P - 1 ) phi_c: %le\n", l->depth, abs_double(diff) );
    }    
      
    END_LOCKED_MASTER(threading)
    if(threading->n_core>1) {
      interpolate3_double( vp1, vc1, l, threading );
      restrict_double( vc2, vp1, l, threading );
      START_LOCKED_MASTER(threading)
      if ( !l->next_level->idle ) {
        vector_double_minus( vc3, vc1, vc2, 0, civs, l->next_level );
        diff = global_norm_double( vc3, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
        test0_double("depth: %d, correctness of ( P* P - 1 ) phi_c with threading: %le\n", l->depth, diff );
      }
      END_LOCKED_MASTER(threading)
    }
    ...
    START_LOCKED_MASTER(threading)
    if (l->depth==0) 
      gamma5_double( vp2, vp1, l, no_threading );
    else
      coarse_gamma5_double( vp2, vp1, 0, ivs, l );
    restrict_double( vc2, vp2, l, no_threading );
    coarse_gamma5_double( vc3, vc2, 0, civs, l->next_level );
    if ( !l->next_level->idle ) {
      vector_double_minus( vc2, vc1, vc3, 0, civs, l->next_level );
      diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
      test0_double("depth: %d, correctness of ( g5_c P* g5 P - 1 ) phi_c: %le\n", l->depth, diff );
    }    
#ifdef HAVE_TM1p1
    if(g.n_flavours == 2) {
      if (l->depth==0) 
        tau1_gamma5_double( vp2, vp1, l, no_threading );
      else
        coarse_tau1_gamma5_double( vp2, vp1, 0, ivs, l );
      restrict_double( vc2, vp2, l, no_threading );
      coarse_tau1_gamma5_double( vc3, vc2, 0, civs, l->next_level );
      if ( !l->next_level->idle ) {
        vector_double_minus( vc2, vc1, vc3, 0, civs, l->next_level );
        diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
        test0_double("depth: %d, correctness of ( tau1 g5_c P* tau1 g5 P - 1 ) phi_c: %le\n", l->depth, diff );
      }    
    }
#endif
    END_LOCKED_MASTER(threading)
    ...
    START_LOCKED_MASTER(threading)
    vector_double_define( vp2, 0, 0, ivs, l );
    if (l->depth==0) 
      add_diagonal_double( vp2, vp1, l->s_double.op.odd_proj, ivs );
    else
      coarse_add_block_diagonal_double( vp2, vp1, l->s_double.op.odd_proj, ivs, l );
    restrict_double( vc2, vp2, l, no_threading );
    
    vector_double_scale( vc2, vc2, -1.0, 0, civs, l->next_level );
    coarse_add_block_diagonal_double( vc2, vc1, l->next_level->s_double.op.odd_proj, civs, l->next_level );
    diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
    test0_double("depth: %d, correctness of ( P* 1odd P - 1odd_c ) phi_c: %le\n", l->depth, diff );
    END_LOCKED_MASTER(threading)  
    ...
    if (g.mu + g.mu_odd_shift != 0.0 || g.mu + g.mu_even_shift != 0.0 ) {
      vector_double_define( vp2, 0, 0, ivs, l );
      if (l->depth==0) 
        add_diagonal_double( vp2, vp1, l->s_double.op.tm_term, ivs );
      else
        coarse_add_anti_block_diagonal_double( vp2, vp1, l->s_double.op.tm_term, ivs, l );
      restrict_double( vc2, vp2, l, no_threading );
      
      vector_double_scale( vc2, vc2, -g.mu_factor[l->next_level->depth]/g.mu_factor[l->depth], 0, civs, l->next_level );
      coarse_add_anti_block_diagonal_double( vc2, vc1, l->next_level->s_double.op.tm_term, civs, l->next_level );
      diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
      test0_double("depth: %d, correctness of ( P* tm P - tm_c ) phi_c: %le\n", l->depth, diff );
    }
    END_LOCKED_MASTER(threading)  
#endif
    ...
    if ( g.n_flavours == 2 &&
      ...
      vector_double_define( vp2, 0, 0, ivs, l );
      if (l->depth==0) 
        apply_doublet_coupling_double( vp2, vp1, l->s_double.op.epsbar_term, ivs );
      else
        coarse_add_doublet_coupling_double( vp2, vp1, l->s_double.op.epsbar_term, ivs, l );
      restrict_double( vc2, vp2, l, no_threading );
      
      vector_double_scale( vc2, vc2, -g.epsbar_factor[l->next_level->depth]/g.epsbar_factor[l->depth], 0, civs, l->next_level );
      coarse_add_doublet_coupling_double( vc2, vc1, l->next_level->s_double.op.epsbar_term, civs, l->next_level );
      diff = global_norm_double( vc2, 0, civs, l->next_level, no_threading ) / global_norm_double( vc1, 0, civs, l->next_level, no_threading );
      test0_double("depth: %d, correctness of ( P* eps P - eps_c ) phi_c: %le\n", l->depth, diff );
    }
    END_LOCKED_MASTER(threading)  
#endif
    ...
    if ( l->level > 0 ) {
      ...
      if (g.mu + g.mu_odd_shift != 0.0 || g.mu + g.mu_even_shift != 0.0 )
        if (g.mu_factor[l->depth] != g.mu_factor[l->next_level->depth]) {  
          vector_double_scale( vp3, vp1, (g.mu_factor[l->next_level->depth]/g.mu_factor[l->depth])-1., 0, ivs, l );
          if(l->depth == 0)
            add_diagonal_double( vp2, vp3, l->p_double.op->tm_term, ivs );
          else
            coarse_add_anti_block_diagonal_double( vp2, vp3, l->p_double.op->tm_term, ivs, l );
        }
#endif      
      restrict_double( vc2, vp2, l, no_threading );
      ...
      if ( !l->next_level->idle ) {
        if ( l->level==1 && g.odd_even )
          coarse_odd_even_double_test( vc3, vc1, l->next_level, no_threading );
        else
          apply_operator_double( vc3, vc1, &(l->next_level->p_double), l->next_level, no_threading );
        
        vector_double_minus( vc3, vc2, vc3, 0, civs, l->next_level );
        diff = global_norm_double( vc3, 0, civs, l->next_level, no_threading ) /global_norm_double( vc2, 0, civs, l->next_level, no_threading );

        if ( l->level==1 && g.odd_even ) {
          test0_double("depth: %d, correctness of odd even preconditioned ( P* D P - D_c ) phi_c: %le\n", l->depth, diff );
        } else {
          test0_double("depth: %d, correctness of ( P* D P - D_c ) phi_c: %le\n", l->depth, diff );
        }
      }      
      END_LOCKED_MASTER(threading)
      ...
      if(threading->n_core>1) {
        if ( !l->next_level->idle ) {
          if ( l->level==1 && g.odd_even )
            coarse_odd_even_double_test( vc3, vc1, l->next_level, threading );
          else
            apply_operator_double( vc3, vc1, &(l->next_level->p_double), l->next_level, threading );
        }
        START_LOCKED_MASTER(threading)
        if ( !l->next_level->idle ) {
          vector_double_minus( vc3, vc2, vc3, 0, civs, l->next_level );
          ...
          if ( l->level==1 && g.odd_even ) { //TODO: this test doesn't work without SSE!!
            test0_double("depth: %d, correctness of odd even preconditioned ( P* D P - D_c ) phi_c with D_c threaded: %le\n", l->depth, diff );
          } else {
            test0_double("depth: %d, correctness of ( P* D P - D_c ) phi_c with D_c threaded: %le\n", l->depth, diff );
          }
        }
        END_LOCKED_MASTER(threading)
        }
    ...
    if ( l->level > 0 && l->depth > 0 && g.method == 3 && g.odd_even ) {
      vector_double_define_random( vp1, 0, ivs, l );
      block_to_oddeven_double( vp4, vp1, l, no_threading );
      coarse_diag_ee_double( vp3, vp4, &(l->oe_op_double), l, no_threading );
      coarse_diag_oo_double( vp3, vp4, &(l->oe_op_double), l, no_threading );
      coarse_hopping_term_double( vp3, vp4, &(l->oe_op_double), _FULL_SYSTEM, l, no_threading );
      oddeven_to_block_double( vp4, vp3, l, no_threading );
      apply_operator_double( vp2, vp1, &(l->p_double), l, no_threading );
      vector_double_minus( vp4, vp4, vp2, 0, ivs, l );
      diff = global_norm_double( vp4, 0, ivs, l, no_threading ) / global_norm_double( vp2, 0, ivs, l, no_threading );
      test0_double("depth: %d, correctness of odd even layout (smoother): %le\n", l->depth, diff );
     
      block_to_oddeven_double( vp4, vp1, l, no_threading );
      coarse_odd_even_double_test( vp3, vp4, l, no_threading );
      oddeven_to_block_double( vp4, vp3, l, no_threading );
      apply_operator_double( vp2, vp1, &(l->p_double), l, no_threading );
      vector_double_minus( vp4, vp4, vp2, 0, ivs, l );
      diff = global_norm_double( vp4, 0, ivs, l, no_threading ) / global_norm_double( vp2, 0, ivs, l, no_threading );
      test0_double("depth: %d, correctness of odd even preconditioned operator (smoother): %le\n", l->depth, diff );
    }

    FREE( vp1, complex_double, 4*vs );
    FREE( vc1, complex_double, 3*cvs );
    END_LOCKED_MASTER(threading)
    
    if ( g.method != 6 && l->next_level->level > 0  && !l->next_level->idle ) {
      schwarz_double_mvm_testfun( &(l->next_level->s_double), l->next_level, threading );
    }

    if ( l->next_level->level > 0 && !l->next_level->idle )
      coarse_operator_double_test_routine( l->next_level, threading );
