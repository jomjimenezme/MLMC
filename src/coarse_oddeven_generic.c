...
void coarse_selfcoupling_LU_decomposition_double( config_double output, operator_double_struct *op, int index, level_struct *l ) {
  ...
  config_double clover = op->clover + n*(n2+1)*index;
  // A
  for ( j=0; j<n; j++ ) {
    for ( i=0; i<j; i++ ) {
      output[n2*i+j] = *clover;
      output[i+n2*j] = conj_double(*clover);
      clover++;      
    }
...
#ifdef HAVE_TM
  config_double tm_term = op->tm_term + n*(n+1)*index;
  if (op->mu + op->mu_odd_shift != 0.0 || op->mu + op->mu_even_shift != 0.0 ) {
    // E
    for ( j=0; j<n; j++ ) {
      for ( i=0; i<j; i++ ) {
        output[n2*i+j] += *tm_term;
        output[i+n2*j] += -conj_double(*tm_term);
        tm_term++;      
      }
...
#ifdef HAVE_TM1p1
void coarse_selfcoupling_LU_doublet_decomposition_double( config_double output, operator_double_struct *op, int index, level_struct *l ) {
  ...
  for ( j=0; j<n; j++ ) {
    for ( i=0; i<n; i++ ) {
      output[n4*(i+0 )+(j+n3)] = _COMPLEX_double_ZERO;
      output[n4*(i+n )+(j+n2)] = _COMPLEX_double_ZERO;
      output[n4*(i+n2)+(j+n )] = _COMPLEX_double_ZERO;
      output[n4*(i+n3)+(j+0 )] = _COMPLEX_double_ZERO;
    }
  }
  ...
  for ( j=0; j<n; j++ ) {
    for ( i=0; i<j; i++ ) {
      ...
      output[i+n4*j] = conj_double(*clover);
      output[n4*(i+n)+(j+n)] = *clover;
      output[(i+n)+n4*(j+n)] = conj_double(*clover);
      clover++;      
    }
  ...
  if (op->mu + op->mu_odd_shift != 0.0 || op->mu + op->mu_even_shift != 0.0 ) {
    ...
    for ( j=0; j<n; j++ ) {
      for ( i=0; i<j; i++ ) {
        ...
        output[i+n4*j] += -conj_double(*tm_term);
        output[n4*(i+n)+(j+n)] -= *tm_term;
        output[(i+n)+n4*(j+n)] -= -conj_double(*tm_term);
        tm_term++;      
      }
  ...
  config_double epsbar_term = op->epsbar_term + n*(n+1)*index;
  // G
  for ( j=n; j<n2; j++ ) {
    for ( i=0; i<(j-n); i++ ) {
      output[n4*i+j] = (*epsbar_term);
      output[(i+n)+n4*(j-n)] = -conj_double(*epsbar_term);
      output[n4*(i+n)+(j-n)] = (*epsbar_term);
      output[i+n4*j] = -conj_double(*epsbar_term);
      epsbar_term++;      
    }
...
void coarse_LU_multiply_double( vector_double y, vector_double x, config_double A, level_struct *l ) {
...
void coarse_diag_ee_double( vector_double y, vector_double x, operator_double_struct *op, level_struct *l, struct Thread *threading ) {
  ...
  coarse_self_couplings_double( y, x, op, start, end, l );
#else
  coarse_self_couplings_double_vectorized( y, x, op, start, end, l );
#endif
}
...
void coarse_diag_oo_double( vector_double y, vector_double x, operator_double_struct *op, level_struct *l, struct Thread *threading ) {
...
#ifdef HAVE_TM1p1
  config_double sc = (g.n_flavours==2) ? op->clover_doublet_oo_inv:op->clover_oo_inv;
#else
  config_double sc = op->clover_oo_inv;
#endif
  ...
  for ( int i=start; i<end; i++ ) {
    coarse_LU_multiply_double( y, x, sc, l );
    x += num_site_var;
    y += num_site_var;
    sc += oo_inv_size;
  }
...
void coarse_diag_double( vector_double y, vector_double x, operator_double_struct *op, level_struct *l ) {
  
  coarse_diag_ee_double( y, x, op, l, no_threading );
  coarse_diag_oo_double( y, x, op, l, no_threading );
}
...
void coarse_diag_oo_inv_double( vector_double y, vector_double x, operator_double_struct *op, 
...
#endif
#else
  int lda = SIMD_LENGTH_double*((num_site_var+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  oo_inv_size = 2*num_site_var*lda;
#ifdef HAVE_TM1p1
  OPERATOR_TYPE_double *sc = (g.n_flavours==2) ? op->clover_doublet_oo_inv_vectorized:op->clover_oo_inv_vectorized;
#else
  OPERATOR_TYPE_double *sc = op->clover_oo_inv_vectorized;
#endif
#endif
  ...
  for ( int i=start; i<end; i++ ) {
#ifndef OPTIMIZED_COARSE_SELF_COUPLING_double
    coarse_perform_fwd_bwd_subs_double( y, x, sc, l );
#else
    for(int j=0; j<num_site_var; j++)
      y[j] = _COMPLEX_double_ZERO;
    cgemv( num_site_var, sc, lda, (double *)x, (double *)y);
#endif
    x += num_site_var;
    y += num_site_var;
    sc += oo_inv_size;
...
void coarse_oddeven_double_set_self_couplings( level_struct *l, struct Thread *threading ) {
  ...
  coarse_operator_double_set_self_couplings( op, l, threading );
  compute_core_start_end_custom( 0, op->num_odd_sites, &start, &end, l, threading, 1);
  ...
  for( int i=start; i<end; i++ )
    coarse_selfcoupling_LU_decomposition_double( op->clover_oo_inv+i*size, op, op->num_even_sites+i, l );

#ifdef HAVE_TM1p1
  int size_doublet = SQUARE(4*nv);
  for( int i=start; i<end; i++ )
    coarse_selfcoupling_LU_doublet_decomposition_double( op->clover_doublet_oo_inv+i*size_doublet, op, 
...
// this else is for vectorized
  ...
  {
    // l->p_double.block_jacobi_double.bj_op_inv_vectorized
    // l->p_double.block_jacobi_double.bj_op_vectorized

    // 1. prepare the operator : B = D_{ee} - I_{eo}*D_{oo}^{-1}*I_{oe}
    OPERATOR_TYPE_double* Dee = op->clover_vectorized;
    OPERATOR_TYPE_double* Dooinv = op->clover_oo_inv_vectorized;
    int startx,endx;
    ...
    for( int i=startx; i<endx; i++ ){
      for( int ix=0;ix<size_v;ix++ )
        l->p_double.block_jacobi_double.bj_op_vectorized[i*size_v+ix] = Dee[i*size_v+ix] - sc_fctr*Dooinv[i*size_v+ix];
    }

    // 2. invert B
    for( int i=startx; i<endx; i++ ){
      cgem_inverse( 2*nv, l->p_double.block_jacobi_double.bj_op_inv_vectorized + i*size_v, 
                    l->p_double.block_jacobi_double.bj_op_vectorized + i*size_v, column_offset );
    }
  }
#endif

#ifdef HAVE_TM1p1
  int column_doublet_offset = SIMD_LENGTH_double*((4*nv+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  ...
  // preparation for direct solves in Block Jacobi
  {
    // l->p_double.block_jacobi_double.bj_doublet_op_inv_vectorized
    // l->p_double.block_jacobi_double.bj_doublet_op_vectorized

    // 1. prepare the operator : B = D_{ee} - I_{eo}*D_{oo}^{-1}*I_{oe}
    OPERATOR_TYPE_double* Dee = op->clover_doublet_vectorized;
    OPERATOR_TYPE_double* Dooinv = op->clover_doublet_oo_inv_vectorized;
    int startx,endx;
    compute_core_start_end_custom( 0, op->num_even_sites, &startx, &endx, l, threading, 1);
    // size of bj_op_vectorized : 2*2*nv*column_offset*op->num_even_sites
    double sc_fctr = 0.0;
    for( int i=startx; i<endx; i++ ){
      for( int ix=0;ix<size_doublet_v;ix++ )
        l->p_double.block_jacobi_double.bj_doublet_op_vectorized[i*size_doublet_v+ix] = Dee[i*size_doublet_v+ix] - sc_fctr*Dooinv[i*size_doublet_v+ix];
    }

    // 2. invert B
    for( int i=startx; i<endx; i++ ){
      cgem_inverse( 4*nv, l->p_double.block_jacobi_double.bj_doublet_op_inv_vectorized + i*size_doublet_v, 
                    l->p_double.block_jacobi_double.bj_doublet_op_vectorized + i*size_doublet_v, column_doublet_offset );
    }
  }
#endif
...
void coarse_oddeven_double_set_couplings( level_struct *l, struct Thread *threading ) {

  coarse_oddeven_double_set_self_couplings( l, threading );
  coarse_operator_double_set_neighbor_couplings( &(l->oe_op_double), l, threading );
...
void coarse_oddeven_alloc_double( level_struct *l ) {
  ...
  operator_double_alloc( op, _ODDEVEN, l );
  ...
  MALLOC( op->clover_oo_inv, complex_double, SQUARE(2*nv)*op->num_odd_sites );
#ifdef HAVE_TM1p1
  MALLOC( op->clover_doublet_oo_inv, complex_double, SQUARE(4*nv)*op->num_odd_sites );
#endif

#else
  int column_offset = SIMD_LENGTH_double*((2*nv+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  MALLOC_HUGEPAGES( op->clover_oo_inv_vectorized, double, 2*2*nv*column_offset*op->num_odd_sites, 4*SIMD_LENGTH_double );
#ifdef BLOCK_JACOBI
  l->p_double.block_jacobi_double.bj_op_inv_vectorized = NULL;
  MALLOC_HUGEPAGES( l->p_double.block_jacobi_double.bj_op_inv_vectorized, double, 2*2*nv*column_offset*op->num_even_sites, 4*SIMD_LENGTH_double );
  l->p_double.block_jacobi_double.bj_op_vectorized = NULL;
  MALLOC_HUGEPAGES( l->p_double.block_jacobi_double.bj_op_vectorized, double, 2*2*nv*column_offset*op->num_even_sites, 4*SIMD_LENGTH_double );
#endif
#ifdef HAVE_TM1p1
  int column_doublet_offset = SIMD_LENGTH_double*((4*nv+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  MALLOC_HUGEPAGES( op->clover_doublet_oo_inv_vectorized, double, 2*4*nv*column_doublet_offset*op->num_odd_sites, 4*SIMD_LENGTH_double );
#ifdef BLOCK_JACOBI
  l->p_double.block_jacobi_double.bj_doublet_op_inv_vectorized = NULL;
  MALLOC_HUGEPAGES( l->p_double.block_jacobi_double.bj_doublet_op_inv_vectorized, double, 2*4*nv*column_doublet_offset*op->num_even_sites, 4*SIMD_LENGTH_double );
  l->p_double.block_jacobi_double.bj_doublet_op_vectorized = NULL;
  MALLOC_HUGEPAGES( l->p_double.block_jacobi_double.bj_doublet_op_vectorized, double, 2*4*nv*column_doublet_offset*op->num_even_sites, 4*SIMD_LENGTH_double );
#endif
#endif
  ...
  // solver
  if ( l->level == 0 )
    l->p_double.v_end = op->num_even_sites*l->num_lattice_site_var;
  else
  ...
  // solver
  if ( l->level == 0 )
    l->p_double.block_jacobi_double.local_p.v_end = op->num_even_sites*l->num_lattice_site_var;
  //else
  //  l->sp_double.v_end = op->num_even_sites*l->num_lattice_site_var;
#endif
...
#ifdef HAVE_TM
  tm_term_double_setup( in->mu, in->mu_even_shift, in->mu_odd_shift, op, l, threading );
#endif  
#ifdef HAVE_TM1p1
  epsbar_term_double_setup( in->epsbar, in->epsbar_ig5_even_shift, in->epsbar_ig5_odd_shift, op, l, threading );
#endif
  
  coarse_oddeven_double_set_couplings( l, threading );
...
void coarse_oddeven_free_double( level_struct *l ) {
  ...
  operator_double_free( op, _ODDEVEN, l );
  coarse_operator_double_free_vectorized( op, l );

#ifndef OPTIMIZED_COARSE_SELF_COUPLING_double

  FREE( op->clover_oo_inv, complex_double, SQUARE(2*nv)*op->num_odd_sites );
#ifdef HAVE_TM1p1
  FREE( op->clover_doublet_oo_inv, complex_double, SQUARE(4*nv)*op->num_odd_sites );
#endif

#else
  int column_offset = SIMD_LENGTH_double*((2*nv+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  FREE_HUGEPAGES( op->clover_oo_inv_vectorized, double, 2*2*nv*column_offset*op->num_odd_sites );
#ifdef BLOCK_JACOBI
  FREE_HUGEPAGES( l->p_double.block_jacobi_double.bj_op_inv_vectorized, double, 2*2*nv*column_offset*op->num_even_sites );
  FREE_HUGEPAGES( l->p_double.block_jacobi_double.bj_op_vectorized, double, 2*2*nv*column_offset*op->num_even_sites );
#endif
#ifdef HAVE_TM1p1
  int column_doublet_offset = SIMD_LENGTH_double*((4*nv+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  FREE_HUGEPAGES( op->clover_doublet_oo_inv_vectorized, double, 2*4*nv*column_doublet_offset*op->num_odd_sites );
#ifdef BLOCK_JACOBI
  FREE_HUGEPAGES( l->p_double.block_jacobi_double.bj_doublet_op_inv_vectorized, double, 2*4*nv*column_doublet_offset*op->num_even_sites );
  FREE_HUGEPAGES( l->p_double.block_jacobi_double.bj_doublet_op_vectorized, double, 2*4*nv*column_doublet_offset*op->num_even_sites );
#endif
#endif
...
#ifdef HAVE_TM1p1
  FREE( op->buffer[0], complex_double, 4*vs );
#else
  FREE( op->buffer[0], complex_double, 2*vs );
#endif
  FREE( op->buffer, complex_double*, 2 );
}
...
void coarse_hopping_term_double( vector_double out, vector_double in, operator_double_struct *op,
  ...
  set_boundary_double( out, 0, l, threading );
  ...
  if ( op->c.comm ) {
    for ( mu=0; mu<4; mu++ ) {
      ...
      ghost_sendrecv_double( in, mu, -1, &(op->c), minus_dir_param, l );
    }
  }
  END_MASTER(threading)
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    coarse_daggered_hopp_double( out_pt, in_pt, D_pt, l );
  }
  SYNC_CORES(threading)
  for ( i=core_start; i<core_end; i++ ) {
    index = 5*i;
  ...
  if ( op->c.comm ) {
    for ( mu=0; mu<4; mu++ ) {
      // communicate in +mu direction
      ghost_sendrecv_double( out, mu, +1, &(op->c), plus_dir_param, l );    
    }
    for ( mu=0; mu<4; mu++ ) {
      // wait for -mu direction
      ghost_wait_double( in, mu, -1, &(op->c), minus_dir_param, l );    
    }
  }
  END_LOCKED_MASTER(threading)
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    coarse_hopp_double( out_pt, in_pt, D_pt, l );
  ...
  if ( op->c.comm ) {
    for ( mu=0; mu<4; mu++ ) {
      // wait for +mu direction
      ghost_wait_double( out, mu, +1, &(op->c), plus_dir_param, l );
    }
  }
  END_LOCKED_MASTER(threading)
...
void coarse_n_hopping_term_double( vector_double out, vector_double in, operator_double_struct *op,
  ...
  int sign = -1;
  coarse_pn_hopping_term_double_vectorized( out, in, op, amount, l, sign, threading);
#else
  coarse_n_hopping_term_double_vectorized( out, in, op, amount, l, threading );
#endif
  return;
#else
  START_NO_HYPERTHREADS(threading)
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    index++;
    out_pt = out + num_site_var*op->neighbor_table[index+T];
    coarse_n_daggered_hopp_double( out_pt, in_pt, D_pt, l );
  }
  SYNC_CORES(threading)
  for ( i=core_start; i<core_end; i++ ) {
    index = 5*i;
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    index++;
    in_pt = in + num_site_var*op->neighbor_table[index+T];
    coarse_n_hopp_double( out_pt, in_pt, D_pt, l );
...
void coarse_hopping_term_double_vectorized( vector_double out, vector_double in, operator_double_struct *op,
  ...
  int column_offset = 2*SIMD_LENGTH_double*((l->num_parent_eig_vect+SIMD_LENGTH_double-1)/SIMD_LENGTH_double);
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    index++;
    out_pt = out + num_site_var*op->neighbor_table[index+T];
    coarse_hopp_double_vectorized( out_pt, in_pt, D_vectorized, l );
  }
  SYNC_CORES(threading)
  for ( i=core_start; i<core_end; i++ ) {
    index = 5*i;
...
void coarse_pn_hopping_term_double_vectorized( vector_double out, vector_double in, operator_double_struct *op,
  ...
  void (*coarse_hopp)(vector_double eta, vector_double phi, OPERATOR_TYPE_double *D, level_struct *l);
  if(sign == +1)
    coarse_hopp = coarse_hopp_double_vectorized;
  else
    coarse_hopp = coarse_n_hopp_double_vectorized;
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    for(int mu=0; mu<4; mu++) {
      // terms coming from backward boundary buffer are done by the ghost_wait_double call below
...
void coarse_n_hopping_term_double_vectorized( vector_double out, vector_double in, operator_double_struct *op,
  ...
  for ( i=core_start; i<core_end; i++ ) {
    ...
    index++;
    out_pt = out + num_site_var*op->neighbor_table[index+T];
    coarse_n_hopp_double_vectorized( out_pt, in_pt, D_vectorized, l );
  }
  SYNC_CORES(threading)
  for ( i=core_start; i<core_end; i++ ) {
    index = 5*i;
...
void coarse_solve_odd_even_double( gmres_double_struct *p, operator_double_struct *op, level_struct *l, struct Thread *threading ) {

  int fgmres_iters=0;

  SYNC_CORES(threading)
  PROF_double_START( _SC, threading );
  coarse_diag_oo_inv_double( p->x, p->b, op, l, threading );
  PROF_double_STOP( _SC, 0, threading );
  PROF_double_START( _NC, threading );
  coarse_n_hopping_term_double( p->b, p->x, op, _EVEN_SITES, l, threading );
  PROF_double_STOP( _NC, 0, threading );

  //double nrmx = global_norm_double( p->b, p->v_start, p->v_end, l, threading );
  ...
  if (g.low_level_meas == 1) {
    ...
    norm_r0 = global_norm_double( p->r, p->v_start, p->v_end, l, threading );
  }

#ifdef BLOCK_JACOBI
  if ( l->level==0 && l->p_double.block_jacobi_double.local_p.polyprec_double.update_lejas == 1 ) {
    // re-construct Lejas
    local_re_construct_lejas_double( l, threading );
  }
#endif

#ifdef POLYPREC
  p->preconditioner = p->polyprec_double.preconditioner;
#endif
  ...
  if ( p->block_jacobi_double.BJ_usable == 1 ) {
    ...
    vector_double_copy( p->block_jacobi_double.b_backup, p->b, start, end, l );
    block_jacobi_apply_double( p->b, p->block_jacobi_double.b_backup, p, l, threading );
  }
#endif

#ifdef GCRODR
  fgmres_iters = flgcrodr_double( p, l, threading );
#else
  fgmres_iters = fgmres_double( p, l, threading );
#endif
  ...
  // restore the rhs
  if ( p->block_jacobi_double.BJ_usable == 1 ) {
    vector_double_copy( p->b, p->block_jacobi_double.b_backup, start, end, l );
  }
#endif
...
//   printf0("fgmres iters = %d\n", fgmres_iters);
...
#ifdef POLYPREC
  if ( l->level==0 && l->p_double.polyprec_double.update_lejas == 1 ) {
    if ( fgmres_iters >= 1.5*p->polyprec_double.d_poly ) {
      // re-construct Lejas
      re_construct_lejas_double( l, threading );
    }
  }
#endif
  ...
  if (g.low_level_meas == 1) {
    ...
    double beta = global_norm_double( p->r, p->v_start, p->v_end, l, threading );
    START_MASTER(threading)
  ...
  // even to odd
  PROF_double_START( _NC, threading );
  coarse_n_hopping_term_double( p->b, p->x, op, _ODD_SITES, l, threading );
...
void coarse_apply_schur_complement_double( vector_double out, vector_double in, operator_double_struct *op, level_struct *l, struct Thread *threading ) {
  ...
  PROF_double_START( _SC, threading );
  coarse_diag_ee_double( out, in, op, l, threading );
  PROF_double_STOP( _SC, 0, threading );

  SYNC_CORES(threading)
  vector_double_define( tmp[0], 0, start, end, l );
  ...
  //END_MASTER(threading)
  PROF_double_START( _NC, threading );
  coarse_hopping_term_double( tmp[0], in, op, _ODD_SITES, l, threading );
  PROF_double_STOP( _NC, 0, threading );
  ...
  PROF_double_START( _SC, threading );
  coarse_diag_oo_inv_double( tmp[1], tmp[0], op, l, threading );
  PROF_double_STOP( _SC, 1, threading );
  ...
  PROF_double_START( _NC, threading );
  coarse_n_hopping_term_double( out, tmp[1], op, _EVEN_SITES, l, threading );
  PROF_double_STOP( _NC, 1, threading );
...
void g5D_coarse_solve_odd_even_double( gmres_double_struct *p, operator_double_struct *op, level_struct *l, struct Thread *threading ) {
  ...
  SYNC_CORES(threading)
  vector_double_define( tmp, 0, start_even, end_even, l );
  
  SYNC_CORES(threading)
  PROF_double_START( _SC, threading );
  coarse_gamma5_double( p->b, p->b, start_odd, end_odd, l );
  SYNC_CORES(threading)
  coarse_diag_oo_inv_double( p->x, p->b, op, l, threading );
  SYNC_CORES(threading)
  coarse_gamma5_double( p->b, p->b, start_odd, end_odd, l );
  PROF_double_STOP( _SC, 0, threading );
  PROF_double_START( _NC, threading );
  coarse_n_hopping_term_double( tmp, p->x, op, _EVEN_SITES, l, threading );
  PROF_double_STOP( _NC, 0, threading );
  coarse_gamma5_double( tmp, tmp, start_even, end_even, l );
  SYNC_CORES(threading)
  vector_double_plus( p->b, p->b, tmp, start_even, end_even, l );

#ifdef GCRODR
  flgcrodr_double( p, l, threading );
#else
  ...
  SYNC_CORES(threading)
  coarse_n_hopping_term_double( tmp, p->x, op, _ODD_SITES, l, threading );
  PROF_double_STOP( _NC, 1, threading );
  SYNC_CORES(threading)
  PROF_double_START( _SC, threading );
  coarse_diag_oo_inv_double( p->b, tmp, op, l, threading );
  vector_double_plus( p->x, p->x, p->b, start_odd, end_odd, l );
...
void g5D_coarse_apply_schur_complement_double( vector_double out, vector_double in, operator_double_struct *op, level_struct *l, struct Thread *threading ) {
  ...
  coarse_gamma5_double( out, out, start_even, end_even, l );
...
void coarse_odd_even_double_test( vector_double out, vector_double in, level_struct *l, struct Thread *threading ) {
  
  if ( g.odd_even ) {
    vector_double buf1 = NULL, buf2 = NULL;
    
    PUBLIC_MALLOC( buf1, complex_double, 2*l->vector_size );
    buf2 = buf1 + l->vector_size;

    START_LOCKED_MASTER(threading)
    // transformation part
    vector_double_copy( buf1, in, 0, l->inner_vector_size, l );
    // even to odd
    vector_double_define( out, 0, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l->inner_vector_size, l );
    END_LOCKED_MASTER(threading)

    coarse_hopping_term_double( out, buf1, &(l->oe_op_double), _ODD_SITES, l, threading );
    coarse_diag_oo_inv_double( buf2, out, &(l->oe_op_double), l, threading );

    START_LOCKED_MASTER(threading)
    vector_double_plus( buf1, buf1, buf2, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l->inner_vector_size, l );
    END_LOCKED_MASTER(threading)
    
    // block diagonal part
    if ( g.method == 6 ) {
      g5D_coarse_apply_schur_complement_double( out, buf1, &(l->oe_op_double), l, threading );
    } else {
      coarse_apply_schur_complement_double( out, buf1, &(l->oe_op_double), l, threading );
    }
    
    coarse_diag_oo_double( out, buf1, &(l->oe_op_double), l, threading );
    ...
    if ( g.method == 6 ) {
      START_LOCKED_MASTER(threading)
      coarse_gamma5_double( out, out, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l->inner_vector_size, l );
      vector_double_define( buf1, 0, 0, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l );
      coarse_hopping_term_double( buf1, buf2, &(l->oe_op_double), _EVEN_SITES, l, no_threading );
      coarse_gamma5_double( buf1, buf1, 0, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l );
      vector_double_plus( out, out, buf1, 0, l->oe_op_double.num_even_sites*l->num_lattice_site_var, l );
      END_LOCKED_MASTER(threading)
    } else {
      coarse_hopping_term_double( out, buf2, &(l->oe_op_double), _EVEN_SITES, l, threading );
    }

    PUBLIC_FREE( buf1, complex_double, 2*l->vector_size );
  }
}
