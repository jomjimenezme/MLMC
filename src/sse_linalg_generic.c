...
void aggregate_gram_schmidt_double_vectorized( complex_double *V, const int num_vec, level_struct *l, struct Thread *threading ) {
    sse_aggregate_gram_schmidt_double( V, num_vec, l, threading );
}
...
void aggregate_gram_schmidt_block_double( double *V,
    ...
    sse_aggregate_gram_schmidt_block_double( V, num_vec, leading_dimension, l, threading );
}
...
void aggregate_block_gram_schmidt_double_vectorized( complex_double *V, const int num_vec, level_struct *l, struct Thread *threading ) {
  ...
  for ( int i=0; i<num_vec; i+=SIMD_LENGTH_double ) {

    int vecs = SIMD_LENGTH_double;
    if(num_vec-i < SIMD_LENGTH_double)
      vecs = num_vec-i;

    for ( int j=0; j<i; j+=SIMD_LENGTH_double )
      aggregate_orthogonalize_block_wrt_orthonormal_block_double( (double *)(V + i*l->vector_size),
          (double *)(V + j*l->vector_size), vecs, l, threading );
    aggregate_gram_schmidt_block_double( (double *)(V + i*l->vector_size), vecs, SIMD_LENGTH_double, l, threading );
  }
  SYNC_CORES(threading)
...
void gram_schmidt_on_aggregates_double_vectorized( complex_double *V, const int num_vec, level_struct *l, struct Thread *threading ) {
  ...
  if(l->depth == 0)
    aggregate_block_gram_schmidt_double_vectorized(V, num_vec, l, threading);
  else
    aggregate_gram_schmidt_double_vectorized(V, num_vec, l, threading);
}


void aggregate_orthogonalize_block_wrt_orthonormal_block_double( double *B, double *U, int num_vec, level_struct *l, struct Thread *threading ) {
  START_NO_HYPERTHREADS(threading)

  double *S = NULL;
  START_LOCKED_MASTER(threading)
  // factors 2 are for complex and spin01/23 aggregates
  MALLOC_HUGEPAGES(S, double, 2*2*l->s_double.num_aggregates*SIMD_LENGTH_double*SIMD_LENGTH_double, 64);
  ((double **)threading->workspace)[0] = S;
  END_LOCKED_MASTER(threading)
  S = ((double **)threading->workspace)[0];

  aggregate_block_dot_block_double(S, U, B, num_vec, SIMD_LENGTH_double, l , threading);
  aggregate_block_minus_block_times_dot_double(B, U, S, num_vec, SIMD_LENGTH_double, l , threading);

  START_LOCKED_MASTER(threading)
  FREE_HUGEPAGES(S, double, 2*2*l->s_double.num_aggregates*SIMD_LENGTH_double*SIMD_LENGTH_double);
  END_LOCKED_MASTER(threading)
...
void aggregate_block_dot_block_double( double *S, double *U, double *B,
    int num_vec, int leading_dimension, level_struct *l, struct Thread *threading ) {
    sse_aggregate_block_dot_block_double( S, U, B, num_vec, leading_dimension, l, threading );
}


void aggregate_block_minus_block_times_dot_double( double *B, double *U, double *S,
    int num_vec, int leading_dimension, level_struct *l, struct Thread *threading ) {
    sse_aggregate_block_minus_block_times_dot_double( B, U, S, num_vec, leading_dimension, l, threading );
}

#ifdef GRAM_SCHMIDT_VECTORIZED_double
    ...
    for ( int j=0; j<count; j++ ) {
      for ( int k=0; k<cache_block_size; k+=12) {
        ...
        for(int m=0; m<3; m++) {
          ...
          sse_complex_deinterleaved_load( (double*)(V[j]+i+k+4*m), &vj_re, &vj_im  );
          sse_complex_deinterleaved_load( (double*)(V[count]+i+k+4*m), &v_re, &v_im  );
    ...
    for ( int j=0; j<count; j++ ) {
      for ( int k=0; k<cache_block_size; k+=12) {
        ...
        for(int m=0; m<3; m++) {
          ...
          sse_complex_interleaved_store(v_re, v_im, (double*)(V[count]+i+k+4*m) ); 
        }
      }
    }
  }
