    ...
    if (l_tmp->level > 0) {
      ...
      if ( mg_params->kcycle_tolerance != g.kcycle_tol ) {
        ...
        if( g.setup_flag || i==0 ) {  
          if ( g.mixed_precision )
            l_tmp->p_double.tol = g.kcycle_tol;
          else
            l_tmp->p_double.tol = g.kcycle_tol;
        }
      }
      ...
      if ( mg_params->coarse_tolerance != g.coarse_tol ){
        ...
        if (g.setup_flag && g.mixed_precision )
          l_tmp->p_double.tol = g.coarse_tol;
...
static inline void DDalphaAMG_driver( double *vector1_out, double *vector1_in, double *vector2_out, double *vector2_in, double tol, DDalphaAMG_status *mg_status, int _TYPE ) {
  ...
  double phase[4] = {_COMPLEX_double_ZERO, _COMPLEX_double_ZERO, _COMPLEX_double_ZERO, _COMPLEX_double_ZERO}, vmin=1, vmax=EPS_double, vtmp, nrhs, nrhs2;
  ...
  if(g.mixed_precision==2 && gvmin/gvmax<EPS_double) {
                                         ...
                                         double  *tol, DDalphaAMG_status *mg_status, int _TYPE ) 
{
  ...
  double phase[4] = {_COMPLEX_double_ZERO, _COMPLEX_double_ZERO, _COMPLEX_double_ZERO, _COMPLEX_double_ZERO},
    vmin=1, vmax=EPS_double, vtmp, nrhs, nrhs2;
  gmres_double_struct *p = g.mixed_precision==2?&(g.p_MP.dp):&(g.p);
...
static inline void DDalphaAMG_proj_driver( double *vector_out, double *vector_in, int level, DDalphaAMG_status *mg_status, int _TYPE ) {
  ...
  vector_double rhs = from->p_double.b;
  vector_double sol = to->p_double.x;
      ...
      for (y=0; y<ll[Y]; y++) 
        for (x=0; x<ll[X]; x++) {
          ...
          for ( mu=0; mu<from->num_lattice_site_var; mu++, j++ )
            rhs[j] = ((complex_double)vector_in[i+2*mu] + I*(complex_double)vector_in[i+2*mu+1]);
        }
  ...
  case _RESTRICT :
    THREADED(threading[0]->n_core)
      restrict_double( sol, rhs, from, threading[omp_get_thread_num()] );
    break;
    
  case _PROLONGATE :
    THREADED(threading[0]->n_core)
      interpolate3_double( sol, rhs, to, threading[omp_get_thread_num()] );
    break;

  case _OPERATOR :
    THREADED(threading[0]->n_core)
      apply_operator_double( sol, rhs, &(from->p_double), from, threading[omp_get_thread_num()] );
    break;
