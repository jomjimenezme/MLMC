...
//   fgmres_double_struct_init( &(p->sp) );
...
//   if ( g.method == 6 ) {
//   p->dp.eval_operator = g5D_plus_clover_double;  p->sp.eval_operator = g5D_plus_clover_double;
//   } else {
//   p->dp.eval_operator = d_plus_clover_double;    p->sp.eval_operator = d_plus_clover_double;
//   }
...
//     p->sp.block_jacobi_double.local_p.v_end = l->inner_vector_size;
//   }
// #endif
// 
//   p->dp.op = &(g.op_double);                     p->sp.op = &(l->s_double.op);
//   
//   g.p.op = &(g.op_double);
//   if ( g.method == 6 ) {
...
//   total = 0;
//   total += (2+m)*vl; // w, V
//   MALLOC( p->sp.V, complex_double*, m+1 );
//   if ( precond != NULL ) {
//     if ( prec_kind == _RIGHT ) {
//       total += (m+1)*vl; // Z
//       k = m+1;
...
//       k = 1;
//     }
//     MALLOC( p->sp.Z, complex_double*, k );
//   }
//   p->sp.total_storage = total;
//   // precomputed storage amount
//   
//   p->sp.w = NULL;
//   MALLOC( p->sp.w, complex_double, total );
//   
//   // reserve storage
//   total = 0;
//   // w
...
//    
//   // single precision
//   FREE( p->sp.w, complex_double, p->sp.total_storage );
//   FREE( p->sp.V, complex_double*, p->sp.restart_length+1 );
//   if ( p->sp.Z != NULL )
//     FREE( p->sp.Z, complex_double*, p->sp.kind==_RIGHT?p->sp.restart_length+1:1 );
//   
//   // double precision
...
// #endif
//     
//     trans_double( p->sp.V[0], p->dp.r, l->s_double.op.translation_table, l, threading );
//     vector_double_real_scale( p->sp.V[0], p->sp.V[0], (double)(1/p->dp.gamma[0]), start, end, l ); // V[0] <- r / gamma_0
//     
...
//                                 
//     trans_back_double( p->dp.r, p->sp.w, l->s_double.op.translation_table, l, threading );
//     if ( ol == 0 ) {
...
// 
// 
// void arnoldi_step_MP( vector_double *V, vector_double *Z, vector_double w,
...
//     MPI_Allreduce( buffer, H[j], j+1, MPI_COMPLEX_double, MPI_SUM, (l->depth==0)?g.comm_cart:l->gs_double.level_comm );
...
// 
//   // orthogonalization
//   complex_double alpha[j+1];
//   for( i=0; i<=j; i++ )
//     alpha[i] = (complex_double) -H[j][i];
//   vector_double_multi_saxpy( w, V, alpha, 1, j+1, start, end, l );
//   
...
//     vector_double_real_scale( V[j+1], w, (double)(1/H[j][j+1]), start, end, l );
// }
// 
// 
// void compute_solution_MP( vector_double x, vector_double *V, complex_double *y,
...
//   
//   // x = V*y
//   vector_double_scale( x, V[0], (complex_double) y[0], start, end, l );
// 
//   complex_double alpha[j];
//   for ( i=1; i<=j; i++ )
//     alpha[i-1] = (complex_double) y[i];
//   vector_double_multi_saxpy( x, &(V[1]), alpha, 1, j, start, end, l );
// }
