...
#ifdef HAVE_TM1p1
  MALLOC( gs->transfer_buffer, complex_double, 2 * gs->dist_inner_lattice_sites * l->num_lattice_site_var );
#else
  MALLOC( gs->transfer_buffer, complex_double, gs->dist_inner_lattice_sites * l->num_lattice_site_var );
#endif  
...
#ifdef HAVE_TM1p1
    MALLOC( gs->buffer, complex_double, 2*l->inner_vector_size );
#else
    MALLOC( gs->buffer, complex_double, l->inner_vector_size );
#endif
...
#ifdef HAVE_TM1p1
    FREE( gs->buffer, complex_double, 2*l->inner_vector_size );
#else
    FREE( gs->buffer, complex_double, l->inner_vector_size );
#endif
  }
...
#ifdef HAVE_TM1p1
  FREE( gs->transfer_buffer, complex_double, 2 * gs->dist_inner_lattice_sites * l->num_lattice_site_var );
#else
  FREE( gs->transfer_buffer, complex_double, gs->dist_inner_lattice_sites * l->num_lattice_site_var );
#endif
}
...
void conf_double_gather( operator_double_struct *out, operator_double_struct *in, level_struct *l ) {
  ...
  int send_size_hopp = l->gs_double.dist_inner_lattice_sites * 4 * SQUARE( l->num_lattice_site_var ),
    send_size_clov = l->gs_double.dist_inner_lattice_sites * ( (l->num_lattice_site_var*(l->num_lattice_site_var+1))/2 ),
    send_size_block = l->gs_double.dist_inner_lattice_sites * ( (l->num_lattice_site_var/2*(l->num_lattice_site_var/2+1)) );
#ifdef HAVE_TM
  out->mu = in->mu;
  ...
  if ( g.my_rank != l->parent_rank ) {
...
#ifdef HAVE_TM1p1
    MPI_Request eps_req;
    MPI_Isend( in->epsbar_term, send_size_block, MPI_COMPLEX_double, l->parent_rank, 4, g.comm_cart, &eps_req );
#endif
#ifdef HAVE_TM
    MPI_Request tm_req;
    MPI_Isend( in->tm_term, send_size_block, MPI_COMPLEX_double, l->parent_rank, 3, g.comm_cart, &tm_req );
#endif
    MPI_Isend( in->odd_proj, send_size_block, MPI_COMPLEX_double, l->parent_rank, 2, g.comm_cart, &odd_req );
    MPI_Isend( in->D, send_size_hopp, MPI_COMPLEX_double, l->parent_rank, 0, g.comm_cart, &req );
    MPI_Send( in->clover, send_size_clov, MPI_COMPLEX_double, l->parent_rank, 1, g.comm_cart );
#ifdef HAVE_TM1p1
    ...
    int i, j, n=l->gs_double.gather_list_length, s=l->num_inner_lattice_sites,
        t, *pi = l->gs_double.permutation;
    vector_double buffer_hopp = NULL, buffer_clov = NULL, buffer_odd_proj = NULL;
    MPI_Request *hopp_reqs = NULL, *clov_reqs = NULL, *odd_proj_reqs = NULL;
    
#ifdef HAVE_TM1p1
    vector_double buffer_eps_term = NULL;
    MPI_Request *eps_term_reqs = NULL;
    MALLOC( buffer_eps_term, complex_double, n*send_size_block );
    MALLOC( eps_term_reqs, MPI_Request, n );
#endif
#ifdef HAVE_TM
    vector_double buffer_tm_term = NULL;
    MPI_Request *tm_term_reqs = NULL;
    MALLOC( buffer_tm_term, complex_double, n*send_size_block );
    MALLOC( tm_term_reqs, MPI_Request, n );
#endif
    MALLOC( buffer_hopp, complex_double, n*send_size_hopp );
    MALLOC( buffer_clov, complex_double, n*send_size_clov );
    MALLOC( buffer_odd_proj, complex_double, n*send_size_block );
    MALLOC( hopp_reqs, MPI_Request, n );
    MALLOC( clov_reqs, MPI_Request, n );
    MALLOC( odd_proj_reqs, MPI_Request, n );
    
    PROF_double_START( _GD_COMM );
    for ( i=1; i<n; i++ ) {
#ifdef HAVE_TM1p1
      MPI_Irecv( buffer_eps_term+i*send_size_block, send_size_block, MPI_COMPLEX_double,
                 l->gs_double.gather_list[i], 4, g.comm_cart, &(eps_term_reqs[i]) );
#endif
#ifdef HAVE_TM
      MPI_Irecv( buffer_tm_term+i*send_size_block, send_size_block, MPI_COMPLEX_double,
                 l->gs_double.gather_list[i], 3, g.comm_cart, &(tm_term_reqs[i]) );
#endif
      MPI_Irecv( buffer_hopp+i*send_size_hopp, send_size_hopp, MPI_COMPLEX_double,
                 l->gs_double.gather_list[i], 0, g.comm_cart, &(hopp_reqs[i]) );
      MPI_Irecv( buffer_clov+i*send_size_clov, send_size_clov, MPI_COMPLEX_double,
                 l->gs_double.gather_list[i], 1, g.comm_cart, &(clov_reqs[i]) );
      MPI_Irecv( buffer_odd_proj+i*send_size_block, send_size_block, MPI_COMPLEX_double,
                 l->gs_double.gather_list[i], 2, g.comm_cart, &(odd_proj_reqs[i]) );      
    }
    PROF_double_STOP( _GD_COMM, 2*n-2 );
...
#ifdef HAVE_TM1p1
    PROF_double_START( _GD_IDLE );
    for ( i=1; i<n; i++ )
      MPI_Wait( &(eps_term_reqs[i]), MPI_STATUS_IGNORE );
    PROF_double_STOP( _GD_IDLE, n-1 );
    ...
    FREE( buffer_hopp, complex_double, n*send_size_hopp );
    FREE( buffer_clov, complex_double, n*send_size_clov );
    FREE( buffer_odd_proj, complex_double, n*send_size_block );
    FREE( hopp_reqs, MPI_Request, n );
    FREE( clov_reqs, MPI_Request, n );
    FREE( odd_proj_reqs, MPI_Request, n );
#ifdef HAVE_TM
    FREE( buffer_tm_term, complex_double, n*send_size_block );
    FREE( tm_term_reqs, MPI_Request, n );
#endif
#ifdef HAVE_TM1p1
    FREE( buffer_eps_term, complex_double, n*send_size_block );
    FREE( eps_term_reqs, MPI_Request, n );
#endif

  }
  l->dummy_p_double.op = out;
  l->dummy_p_double.v_start = 0;
  l->dummy_p_double.v_end = l->inner_vector_size;
  
  if ( g.method == 6 )
    l->dummy_p_double.eval_operator = g5D_apply_coarse_operator_double;
  else
    l->dummy_p_double.eval_operator = apply_coarse_operator_double;
}

void vector_double_gather( vector_double gath, vector_double dist, level_struct *l ) {
  
  int send_size = l->gs_double.dist_inner_lattice_sites * l->num_lattice_site_var;
  
  if ( g.my_rank != l->parent_rank ) {
    MPI_Send( dist, send_size, MPI_COMPLEX_double, l->parent_rank, g.my_rank, g.comm_cart );
  } else {
    int i, j, n=l->gs_double.gather_list_length, s=l->num_inner_lattice_sites,
        t=l->num_lattice_site_var, *pi = l->gs_double.permutation;
    vector_double buffer = l->gs_double.buffer;

    PROF_double_START( _GD_COMM );
    for ( i=1; i<n; i++ )
      MPI_Irecv( buffer+i*send_size, send_size, MPI_COMPLEX_double, l->gs_double.gather_list[i],
                 l->gs_double.gather_list[i], g.comm_cart, &(l->gs_double.reqs[i]) );
    PROF_double_STOP( _GD_COMM, n-1 );
    ...
    PROF_double_START( _GD_IDLE );
    for ( i=1; i<n; i++ )
      MPI_Wait( &(l->gs_double.reqs[i]), MPI_STATUS_IGNORE );
    PROF_double_STOP( _GD_IDLE, n-1 );
...
void vector_double_distribute( vector_double dist, vector_double gath, level_struct *l ) {
  ...
  if ( g.my_rank != l->parent_rank ) {
    MPI_Recv( dist, send_size, MPI_COMPLEX_double, l->parent_rank, g.my_rank, g.comm_cart, MPI_STATUS_IGNORE );
  } else {
    int i, j, n=l->gs_double.gather_list_length, s=l->num_inner_lattice_sites,
        t=l->num_lattice_site_var, *pi = l->gs_double.permutation;
    vector_double buffer = l->gs_double.buffer;
    ...
    PROF_double_START( _GD_COMM );
    for ( i=1; i<n; i++ )
      MPI_Isend( buffer+i*send_size, send_size, MPI_COMPLEX_double, l->gs_double.gather_list[i], 
                 l->gs_double.gather_list[i], g.comm_cart, &(l->gs_double.reqs[i]) );
    PROF_double_STOP( _GD_COMM, n-1 );
